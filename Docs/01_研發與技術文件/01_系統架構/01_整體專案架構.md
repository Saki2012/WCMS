...
- 作為 Browser-facing Security Boundary，統一處理 Node／Proxy 對外安全標頭與 Browser Runtime 契約。
- Backend 建立的 Cookie 原則上保留其 `Set-Cookie` 安全屬性，不在 Node 另外維護第二套 SameSite Policy。
...
- CMS 資料 AA 檢查。
- 舊資料匯入與初始化。
- 作為 Cookie、XSRF、Authentication／Authorization 等 Server-side Security Policy Authority 與 Enforcement 邊界。
...
`Default Policy` 是一般情境的保守安全基準；Feature／Spec 只描述必要業務需求，不直接在局部程式碼中任意降低全域安全 Policy。相容性或 Cross-Site Requirement 若確實存在，應由對應的 Security Owner 建立具名例外，再完成實際 Runtime 與弱掃驗證。

### 7.1 前端層

前端 `SysCore/Security` 的主要定位是 Browser Security Boundary／Contract。前端 SSR Server 目前處理：
...
Frontend Security 主要負責 Browser-facing Response、Node／Proxy Header Finalization 與 Browser Runtime Security；不另外建立與 Backend 重複的 Authentication、Authorization 或 Cookie SameSite Policy Authority。

### 7.2 後端層

後端 `SysCore/Security` 是 Server-side Security Policy Authority／Enforcement。後端目前處理：
...
Backend Security 負責真正會改變 Server-side Security Contract 的 Policy，例如 Authentication／Authorization、XSRF 與 Cookie SameSite；Feature 不直接擁有這些全域安全等級。

### 7.3 前後端 Security Authority 與 Cookie 流程

安全能力必須先確認 Policy Owner，再確認 Browser 最終結果。整體責任如下：

```text
Feature / Spec / Platform Capability
        │
        │ 只描述 Business / Security Requirement
        ▼
Security Policy Owner
        │
        ├─ Frontend SysCore/Security
        │    → Browser-facing Header / Runtime Boundary
        │
        └─ Backend SysCore/Security
             → Cookie / XSRF / Authentication / Authorization Policy
        │
        ▼
Default Policy / Named Exception
        │
        ▼
Middleware / Proxy / Framework Enforcement
        │
        ▼
Browser Final Response / Runtime
```

Cookie 是目前第一個需要明確套用此跨層模型的案例。目標流程為：

```text
Feature / Platform Capability
→ 建立或取得某個 Cookie Role
→ Backend Cookie Security Policy
   ├─ 無 Cross-Site Requirement：Default Strict
   ├─ Top-level Safe Navigation Continuity：Named Lax Exception
   └─ 真正 Cross-Site Requirement：Explicit None + Secure Exception
→ ASP.NET Cookie Policy / Middleware Enforcement
→ Backend Set-Cookie
→ Frontend Node /Service Proxy 原則上保留 Cookie Security Attribute
→ Browser Final Set-Cookie / Runtime 驗證
```

因此 `Strict`、`Lax`、`None` 不是由一般 Feature 自行挑選的同級設定；`Strict` 是預設安全基準，`Lax`／`None` 必須源自明確跨站需求與中央 Security Policy 例外。Frontend 負責 Browser Contract 與 Proxy 邊界，Backend 則是 SameSite 的 Policy Authority。

> [!IMPORTANT]
> 上述 Cookie Security Flow 是 2026-08-28 已確認的目標架構。現行 `Feature/Dev` 仍有部分 Feature Source 宣告 `SameSite=Lax`、Global Cookie Policy 最低 `Strict` 的歷史 Policy Drift，實際 Code 尚待收斂。因此本文件更新不代表 Cookie 弱掃 Finding 已完成 Code mitigation、Runtime 驗證或正式 Scanner 複掃。

Cookie、Header、CSP、CORP、XSRF 等 Security Finding 均應遵守相同治理原則：先確認 Owner 與 Default，再處理具名例外；不可只因 Scanner Finding 在任一層看到症狀，就直接於該 Feature 或 Proxy 局部加補丁。

詳細責任與實作方向分別參考：

- [`02_前端專案架構.md`](./02_前端專案架構.md) 的 `SysCore Security Hardening`。
- [`03_後端專案架構.md`](./03_後端專案架構.md) 的 `XSRF、CORS 與 Security Hardening`／`Cookie Security Policy`。

### 7.4 弱點掃描責任

前端 Node 與後端 ASP.NET Core 都可能影響 Browser 最終 Security Response。正式部署時必須確認：

- 對外最終 Response 的標頭由哪一層負責。
- Proxy 不得輸出互相衝突的 CSP／HSTS。
- IIS、Node、Backend 不得重複洩漏 Server 資訊。
- Cookie Finding 必須檢查 Browser-facing 最終 `Set-Cookie`，不能只看 Controller 內的 `CookieOptions`。
...
### 7.5 後台 Auth Session、Shared State 與併發原則
...
- F5、Ctrl+R 等由使用者實際 `keydown` 觸發的重新整理可視為有效 Activity，因此可以重設 Idle；重新載入後的 Coordinator mount 本身不得再額外建立新的 LastActivity。
- 新開 Tab、Browser Reopen、`/Auth/Me`、Refresh Rotation、focus／visibilitychange 本身不得自行重設 Idle。
...
12. 跨層 Security Policy 必須明確定義 Owner、Default、Named Exception 與 Final Validation；Feature／Spec 不得自行降低全域安全基準。
...
WCMS_Frontend/src/SysCore/Security/Hardening/SecurityHeaders.ts
...
WCMS_Backend/SysCore/Security/Hardening/SecurityHeadersSetup.cs
WCMS_Backend/SysCore/Security/Hardening/XsrfProtectionSetup.cs
