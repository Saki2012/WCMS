...
public class SurveyController : ApiDataController<SurveySet, SurveySet_DTO>
{
    ...
    public async Task<IActionResult> Public_Submit([FromBody] SurveySubmissionSubmit_DTO data, CancellationToken ct)
    {
        SurveySubmissions submit = BuildPublicSubmitData(data);
        await ((SurveyBiz)Service).SubmitSurvey(submit, data.FormDataJson, ct);
    ...
    /// 建立前台提交資料
    /// </summary>
    protected virtual SurveySubmissions BuildPublicSubmitData(SurveySubmissionSubmit_DTO data)
    {
    ...
    /// 建立提交回應
    /// </summary>
    protected virtual IActionResult BuildSubmitResponse(SurveySubmissions submit)
    {
    ...
    /// 自動補使用者端資訊
    /// </summary>
    protected virtual void AutoSetClientInfo(SurveySubmissions submit)
    {
    ...
    /// 取得代理轉發 IP
    /// </summary>
    protected virtual string GetForwardedIp()
    {
    ...
    /// 遮罩使用者 IP
    /// </summary>
    protected virtual string MaskClientIp(string ip)
    {
    ...
    /// 遮罩 IPv4
    /// </summary>
    protected virtual string MaskIPv4(IPAddress address)
    {
    ...
    /// 遮罩 IPv6
    /// </summary>
    protected virtual string MaskIPv6(IPAddress address)
    {
        byte[] bytes = address.GetAddressBytes();
    ...
    /// 雜湊使用者 IP
    /// </summary>
    protected virtual string HashClientIp(string ip)
    {
        if (string.IsNullOrWhiteSpace(ip)) return string.Empty;
    ...
    /// 截斷文字避免超過資料欄位長度
    /// </summary>
    protected virtual string CutText(string? value, int maxLength)
    {
