    ...
    where TFormModel : class
{
    ...
    /// </summary>
    private void NormalizeForeignKeyValues(HeaderModel header, IReadOnlyList<IList> detailLists)
    {
    ...
    /// </summary>
    private async Task EnsureBusinessIdAsync(HeaderModel header, IReadOnlyList<IList> detailLists, CancellationToken ct)
    {
    ...
    /// </summary>
    private async Task<string> GenerateBusinessIdAsync(Type modelType, PropertyInfo keyProperty, CancellationToken ct)
    {
        LambdaExpression selector = BuildIdSelectorLambda(modelType, keyProperty);
        string dailyPrefix = BusinessIdGenerator.BuildDailyPrefix(PrefixAccessor(), DateTime.Now);
    ...
    /// </summary>
    private void PropagateHeaderKey(IReadOnlyList<IList> detailLists, string keyName, object? keyValue)
    {
        foreach (IList rows in detailLists)
            foreach (object? row in rows)
    ...
    /// </summary>
    private void PreserveKey(DbModel oldRoot, DbModel newRoot, IEnumerable<object> details, PropertyInfo key)
    {
    ...
    /// 建立業務編號欄位 Selector。
    /// </summary>
    private static LambdaExpression BuildIdSelectorLambda(Type modelType, PropertyInfo property)
    {
        ...
        Type delegateType = typeof(Func<,>).MakeGenericType(modelType, typeof(string));
