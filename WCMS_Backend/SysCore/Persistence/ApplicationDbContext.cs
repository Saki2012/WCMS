...
namespace WCMS.SysCore.Persistence
{
    ...
    public static class LibDataAccess
    {
        ...
        public static readonly IConfiguration Configuration = new ConfigurationBuilder().SetBasePath(AppContext.BaseDirectory)
            .AddJsonFile(SysParam.Configuration.AppSettingsFile, optional: false, reloadOnChange: true)
            .AddJsonFile($"appsettings.{Environment.GetEnvironmentVariable(SysParam.EnvironmentVariables.AspNetCoreEnvironment) ?? "production"}.json", optional: true, reloadOnChange: true).Build();
        #endregion
        #region Public
        /// <summary>
        /// 
        ...
        private static DbContextOptions<ApplicationDbContext> GetConnectionOption(IConfiguration config)
        {
            ...
            builder.UseSqlServer(config.GetConnectionString(SysParam.Configuration.ConnectionStrings.SqlConnection), b => b.MigrationsAssembly(nameof(WCMS)));
