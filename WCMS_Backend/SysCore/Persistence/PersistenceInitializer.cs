...
public static class PersistenceInitializer
{
    ...
    /// </summary>
    public static async Task EnsureDbSpecCodeAsync(ApplicationDbContext db, CancellationToken ct = default)
    {
        string appSpecCode = SpecSettings.SpecCode?.Trim() ?? string.Empty;
        SysDbProfile? profile = await db.SysDbProfile.FirstOrDefaultAsync(p => p.ProfileKey == SpecCodeKey, ct);
        if (profile == null)
        {
            await CreateDbProfileAsync(db, appSpecCode, ct);
            return;
        }
    ...
    /// </summary>
    public static async Task RegisterUdfAsync(IConfiguration cfg, IWebHostEnvironment env, ApplicationDbContext db, CancellationToken ct = default)
    {
        ...
        try
        {
            await ExecuteUdfFilesAsync(connection, files, ct);
        }
        finally
        {
    ...
    /// </summary>
    private static async Task CreateDbProfileAsync(ApplicationDbContext db, string appSpecCode, CancellationToken ct)
    {
...
目前程式與資料庫不屬於同一個 Spec，請確認升級目標 DB、連線字串、SpecCode 設定是否正確。");
    ...
    /// 依序執行所有 UDF SQL 檔案。
    /// </summary>
    private static async Task ExecuteUdfFilesAsync(SqlConnection connection, IEnumerable<string> files, CancellationToken ct)
    {
        foreach (string file in files) await ExecuteUdfFileAsync(connection, file, ct);
    }
    /// <summary>
    /// 以獨立交易執行單一 UDF SQL 檔案。
    /// </summary>
    private static async Task ExecuteUdfFileAsync(SqlConnection connection, string file, CancellationToken ct)
    {
        string[] batches = await ReadSqlBatchesAsync(file, ct);
        using SqlTransaction transaction = connection.BeginTransaction();
        try
        {
            await ExecuteSqlBatchesAsync(connection, transaction, batches, ct);
    ...
    /// </summary>
    private static async Task<string[]> ReadSqlBatchesAsync(string file, CancellationToken ct)
    {
        string sqlText = await File.ReadAllTextAsync(file, Encoding.UTF8, ct);
    ...
    /// 在指定交易中依序執行 SQL 批次。
    /// </summary>
    private static async Task ExecuteSqlBatchesAsync(SqlConnection connection, SqlTransaction transaction, IEnumerable<string> batches, CancellationToken ct)
    {
