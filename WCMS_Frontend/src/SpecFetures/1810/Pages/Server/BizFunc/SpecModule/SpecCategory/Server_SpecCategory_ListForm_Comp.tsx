...
/** 建立空資料（新增模式用） */
const buildEmptySet = (progId: string): SpecCategorySet => ({
  SpecCategory: { ProgId: progId },
  SpecCategoryDetail: [],
});

/** 類別（SpecCategory）List/Form 主頁（抽離 FetchData 到 Hook） */
export const Server_SpecCategoryListFormComp = (prop: {
  progId: SchemaFields.PGID;
  title: string;
  theme: IBETheme;
  lang: Lang;
}) => {
  ...
  const getData = useSpecCategoryListFormFetchData({
    dirUrl,
    lang: prop.lang,
  ...
  const tagEditNode = useMemo(
    () =>
      getData.rawData.editForm ? (
        <SpecCateEditComp
          theme={prop.theme}
          formData={getData.rawData.editForm}
          showCols={getData.rawData.showCols}
        />
      ) : null,
  ...
  const tagListNode = useMemo(
    () =>
      getData.rawData.list ? (
        <SpecCateListComp
          theme={prop.theme}
          SpecCateSets={getData.rawData.list}
          lang={prop.lang}
...
const SpecCateEditComp = (props: {
  ...
  const tabContent: Record<string, React.ReactNode[]> = rawDetails.reduce<
  ...
  >((compMap, info) => {
    ...
    compMap[langKey] = [
      <LibTextBox
        ...
        {...setField(
          SchemaFields.SpecCategorySetFields.SpecCategoryDetail,
          SchemaFields.SpecCategoryDetailModelFields.CategoryName,
          "string",
          rowKeys,
        )}
      />,
      <LibCheckBox
        ...
        {...setField(
          SchemaFields.SpecCategorySetFields.SpecCategory,
...
/** 清單區塊（維持原 UL/LI 結構） */
const SpecCateListComp = (prop: {
  theme: IBETheme;
  SpecCateSets: SpecCategorySet[];
  lang: Lang;
      ...
      {prop.SpecCateSets.map((item) => {
        ...
        return (
          <li
            ...
            key={`${item.SpecCategory?.InternalId}-${
              item.SpecCategoryDetail?.find((p) => p.Lang === prop.lang)?.RowId
            }`}
          >
