...
import { useSpecUSRListFetchData, type SpecUSRListRawData } from "./Server_SpecUSR_List_Hook";
...
export const Server_SpecUSR_List_Comp = (prop: { title: string; theme: IBETheme; lang: Lang }) => {
    ...
    // return
    return <ListComp Title={prop.title} Theme={prop.theme} isLoading={getData.isLoading} ErrorList={getData.errors} Actions={listActions} GridData={gridData} SearchBar={searchCompProp}/>
};
...
const useSpecUSRListToolbarActions = (dirUrl: string): UseActionsResult => {
    ...
    return useMemo(
        () => ({
            isExecuting: false,
            onSave: async () => false,
            onDelete: async () => { /* list toolbar 不用 */ },
            onInvalid: () => { /* list toolbar 不用 */ },
            onCancelBack,
            onAddNew,
            onEdit,
            onPreview: () => { /* list toolbar 不用 */ },
        }),
...
const buildSpecUSRGridProps = (opt: {
    ...
    const visibleCols = [SpecUSRModelFields.CategoryId, SpecUSRModelFields.Tags, SpecUSRDetailFields.Year,
        SpecUSRDetailFields.AcademicYear, SpecUSRDetailFields.ProjectName, SpecUSRDetailFields.ProjectConcept,
        SpecUSRModelFields.CreateTime, SpecUSRModelFields.ModifyUserId, SpecUSRModelFields.ModifyTime,
    ];
    // 執行 function：Grid 基礎資料
    const columns = buildColumns(visibleCols, opt.raw);
    const rows = buildSpecUSRRows(opt.raw, opt.lang, columns);
    const baseGrid: GridProps = { columns, rows, 
