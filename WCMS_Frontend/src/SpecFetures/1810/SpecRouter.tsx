...
export const specClientEntries: Record<string, ModuleEntry> = {
    SpecUSR: {
        ...
        children: (opts: unknown, lang: Lang) => [{
            index: true,
            loader: withRequestLang((reqLang) => SpecUSRList_Loader({ lang: reqLang, opts: opts as ISpecUSRListOptions })),
            element: <SpecUSRListComp Theme={Classic_FETheme} Lang={lang} Options={opts as ISpecUSRListOptions} />,
        }, { path: ":internalId", loader: SpecUSRForm_Loader(), element: <SpecUSRFormComp Theme={Classic_FETheme} Lang={lang} /> }],
    },
    SpecResearch: {
        kind: "routes",
        element: (lang: Lang, site: INormSite, node: INormNode) => <SubPage style={Classic_FETheme} lang={lang} site={site} node={node} />,
        children: (opts: unknown, lang: Lang) => [{
            index: true,
            loader: withRequestLang((reqLang) => SpecResearchList_Loader({ lang: reqLang, opts: opts as ISpecResearchListOptions })),
            element: <SpecResearchListComp Theme={Classic_FETheme} Lang={lang} Options={opts as ISpecResearchListOptions} />,
        }],
    },
};
