...
import { findTextByKey, LibCondition, Operator } from "@/SysCore/Utils/Library/LibData";
...
export const Client_SpecProduction_List_Loader = (p: { lang: Lang; opts?: Module_SpecProduction_OptionsJson; }) => async ({ request }: LoaderFunctionArgs): Promise<SpecProductionListLoaderData> =>
{
...
const buildMaterialParam = (p: { categoryId?: string; tagIds?: string; }): QueryListParam | null =>
{
    ...
    return {
        ...
        Condition: LibCondition.joinConditions(
            [LibCondition.createCondition(MaterialFields.CategoryId, Operator.Equal, p.categoryId), LibCondition.createCondition(`${MaterialFields._MaterialTags}.${MaterialTagsFields.TagId}`, Operator.In, p.tagIds)],
        ),
