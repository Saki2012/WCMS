import { CalendarAdapterImpl, CalendarService } from "@/Features/Hooks/BizFunc/SystemSetting/Calendar_Api";
...
type SpecExtraLoaders = {
    getCurrentOpenTimeLoader: (opt?: {getApiInstance?: (args: LoaderFunctionArgs) => AxiosInstance | undefined;}) 
        => (args: LoaderFunctionArgs) => Promise<ApiLoaderData<null, CurrentOpenTime[]>>;
};

type SpecExtraHooks = {
    useFetchCurrentOpenTime: (opt?: { initial?: ApiLoaderData<null, CurrentOpenTime[]> | null; deps?: EffectDeps; onError?: (err: ApiAdapterError) => void; apiInstance?: AxiosInstance; }) 
        => { data: CurrentOpenTime[]; apiRes: ApiResponse<CurrentOpenTime[]> | null; isLoading: boolean; errorText: string | null; refetch: () => Promise<void>; };
};

class SpecCalendarAdapterImpl extends CalendarAdapterImpl
{
    // #region Property
    public declare loader: CalendarAdapterImpl["loader"] & SpecExtraLoaders;
    public declare hooks: CalendarAdapterImpl["hooks"] & SpecExtraHooks;
    // #endregion
    ...
    protected override buildExtendedLoader(base: ApiDataLoaderGroup<CalendarSet>)
    {
        ...
        const wrapGetCurrentOpenTimeLoader: SpecExtraLoaders["getCurrentOpenTimeLoader"] = (opt) =>{ return this.getCurrentOpenTimeLoader(opt); };
        return {...merged, 
            getCurrentOpenTimeLoader: wrapGetCurrentOpenTimeLoader,
        };
    }
    protected override buildExtendedHooks(base: ApiDataHookGroup<CalendarSet>)
    {
        const merged = super.buildExtendedHooks(base);
        const wrapUseFetchCurrentOpenTime: SpecExtraHooks["useFetchCurrentOpenTime"] = (opt) => { return this.useFetchCurrentOpenTime(opt); };
        return {...merged,
            useFetchCurrentOpenTime: wrapUseFetchCurrentOpenTime,
        };
    }
    ...
    // #region Hook Func
    private useFetchCurrentOpenTime(opt?: { initial?: ApiLoaderData<null, CurrentOpenTime[]> | null; deps?: EffectDeps; onError?: (err: ApiAdapterError) => void; apiInstance?: AxiosInstance; })
    {
        const deps = opt?.deps ?? [];
...
export const SpecCalendarAdapter = (apiInstance?: AxiosInstance) => new SpecCalendarAdapterImpl((api?: AxiosInstance) => new SpecCalendarService(api ?? apiInstance));