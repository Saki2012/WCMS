...
//#region Public
export type ScheduleRuleFormRawData = {
    formData: UseFetchFormDataResult<SpecOpenScheduleRuleSet>;
    actions: ServerFormActions;
};

export type ScheduleRuleFormAdapter = {
    ScheduleRule: ReturnType<typeof SpecOpenScheduleRuleAdapter>;
};

/** ✅ 主入口：Server ScheduleRule Form 的所有「讀取資料 / actions」集中出口 */
export const useScheduleRuleFormFetchData = (opt: {
    internalId: string;
    emptyData: SpecOpenScheduleRuleSet;
    onBackToList: () => void;
    ...
    const formData = useScheduleRuleFormDataByAdapter(
        adapter.ScheduleRule,
        opt.internalId,
        opt.emptyData,
        onError,
    );
    ...
    const actions = useScheduleRuleFormActionsByAdapter(
        adapter.ScheduleRule,
        opt.internalId,
        formData.data,
        opt.onBackToList,
...
const useScheduleRuleFormDataByAdapter = (
    ...
    const initial = useMemo<ApiLoaderData<string, SpecOpenScheduleRuleSet> | null>(() => {
        ...
        const apiRes: ApiResponse<SpecOpenScheduleRuleSet> = {
            IsSuccess: true,
            Data: empty,
            SysMessage: [],
        };
