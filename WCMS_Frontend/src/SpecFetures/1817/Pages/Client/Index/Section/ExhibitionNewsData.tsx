...
const buildCategoryDict = (
  ...
  return rows.reduce<Record<string, string>>((acc, row) => {
    ...
    const name =
      row.CategoryDetail?.find((item) => item.Lang === lang)?.CategoryName ??
      "";
...
const buildTagDict = (rows: TagSet[], lang: Lang): Record<string, string> => {
  ...
  return rows.reduce<Record<string, string>>((acc, row) => {
    ...
    const name =
      row.TagDetail?.find((item) => item.Lang === lang)?.TagName ?? "";
...
const getNewsDataProps = (
  ...
  return newsData.slice(0, 6).map((item) => {
    ...
    return {
      ...
      title:
        item.AnnouncementDetail?.find((p) => p.Lang === lang)?.Title ?? "",
      content:
        item.AnnouncementDetail?.find((p) => p.Lang === lang)?.Content ?? "",
      date: date.day,
      month: date.month,
      year: date.year,
      monthNum,
