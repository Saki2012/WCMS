import type { SpecHomePage1821Banner } from "@/SpecFetures/1821/Hooks/WEB/HomePage_Types";
...
// #region Property
type BannerModel = SpecHomePage1821Banner;
// #endregion
...
export const Section1 = (props: { lang: Lang; data: BannerModel[]; }) =>
{
    ...
    return (
        <section className="spec1821-banner" aria-label="首頁主視覺輪播">
            <div className="spec1821-banner__media">
                <BannerMediaSection item={activeItem} lang={props.lang} />
            </div>
            <BannerCaptionSection item={activeItem} />
            {banners.length > 1 && (
                <BannerControlsSection
                    banners={banners}
                    safeIndex={safeIndex}
                    onPrev={handlePrev}
                    onNext={handleNext}
...
const BannerMediaSection = (props: { item: BannerModel; lang: Lang; }) =>
{
    const image = (
        <img
            src={FileManagementAPI.get_Public_Preview_Url(props.item.BannerFileId)}
            alt={getBannerAlt(props.item)}
        />
    );
    ...
    return (
        <LangLink
            ...
            aria-label={getBannerLinkLabel(props.item)}
        >
            {image}
        </LangLink>
    );
...
/** Banner 控制列區塊 */
const BannerControlsSection = (props: {
    banners: BannerModel[];
    safeIndex: number;
    onPrev: () => void;
    onNext: () => void;
...
{
    return (
        <div className="spec1821-banner__controls">
            <button type="button" onClick={props.onPrev} aria-label="上一張 Banner">
                ‹
            </button>
            {props.banners.map((item, index) => (
                <BannerIndicator
                    ...
                    item={item}
                    index={index}
                    active={index === props.safeIndex}
                    onSelect={props.onSelect}
                />
            ))}
            <button type="button" onClick={props.onNext} aria-label="下一張 Banner">
                ›
            </button>
        </div>
    );
