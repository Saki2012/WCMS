...
export const specClientEntries: Record<string, ModuleEntry> = {
  SpecJournal: {
    ...
    children: (opts, lang, node: INormNode) => [
      ...
      // 1) Index：年份/卷期索引
      { path: "Index", element: <SpecJournalIndex node={node} lang={lang} />, handle: { breadcrumb: "journal-index" }, },
      // 2) List：某一期
      { path: "List/:indexId?/:rowId?", element: <SpecJournalList node={node} lang={lang} />, handle: { breadcrumb: "journal-issue" }, },
      // 3) Form：某一篇（articleId 必填）
      { path: "Form/:indexId/:rowId/:journalId?", element: <SpecJournalForm_Comp node={node} lang={lang} />, handle: { breadcrumb: "journal-article" }, },
    ],
  },
};
