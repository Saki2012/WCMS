...
export const HomePageLoader = (p: { lang: Lang; }) => async ({ request }: LoaderFunctionArgs): Promise<HomePageLoaderData> =>
{
    ...
    // 宣告變數：整理回傳資料
    const latestIssueBgBanner = takeFirstOrNull<BannerFormModel>(getEnv(latestIssueBgLD).Data);
    const latestIssueCoverBanner = takeFirstOrNull<BannerFormModel>(getEnv(latestIssueCoverLD).Data);
    ...
    const indexedBanner = takeFirstOrNull<BannerFormModel>(getEnv(indexedLD).Data);
...
const toBannerList = (data?: BannerFormModel | null): BannerFormModel[] =>
{
    return data ? [data] : [];
};
...
const useHomePageDataLegacy = (props: UseHomePageDataProps): UseHomePageDataResult =>
{
    ...
    const rawData = useMemo<HomePageRawData | null>(() =>
    {
        ...
        return {
            latestIssueBgBanner: takeFirstOrNull<BannerFormModel>(latestIssueBgQuery.data),
            latestIssueCoverBanner: takeFirstOrNull<BannerFormModel>(latestIssueCoverQuery.data),
            latestIssuePublishedList: latestIssuePublishedQuery.data ?? [],
            latestIssueUnpublishedList: latestIssueUnpublishedQuery.data ?? [],
            indexedBanner: takeFirstOrNull<BannerFormModel>(indexedQuery.data),
            newsTopList,
            newsList,
