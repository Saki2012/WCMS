...
import { useSpecJournalIndexListFetchData, type SpecJournalIndexListAdapter, } from "./Server_SpecJournalIndex_List_Hook";
...
export const Server_SpecJournalIndex_List_Comp = (prop: { title: string; theme: IBETheme; lang: Lang; }) =>
{
    ...
    const columns = useMemo<ColumnConfig[]>(() => { return BuildVisibleColumnsFromSchema(fetchData.rawData.modelDisplayName, visibleKeys); }, [fetchData.rawData.modelDisplayName, visibleKeys]);
    const rows = useMemo<GridRow[]>(() => { return (fetchData.rawData.list ?? []).map((item) => ParseSpecJournalIndexRow(item, columns)); }, [fetchData.rawData.list, columns]);
    const baseGrid = useMemo<GridProps>(() => { return {columns, rows, rawData: fetchData.rawData.list ?? [], CurrentPage: fetchData.rawData.pageNumber, TotalPage: fetchData.rawData.totalPages, onPageChange: fetchData.rawData.onPageChange, };}, [columns, rows, fetchData.rawData.list, fetchData.rawData.pageNumber, fetchData.rawData.totalPages, fetchData.rawData.onPageChange, ]);
    const adjustedGrid = useMemo<GridProps>(() => { return SetAdjustFunction(baseGrid, fetchData.rawData.list, actions); }, [baseGrid, fetchData.rawData.list, actions]);
    return <ListComp Title={prop.title} Theme={prop.theme} isLoading={fetchData.isLoading} ErrorList={fetchData.errors} Actions={actions} GridData={adjustedGrid} SearchBar={searchCompProp}/>
};
...
const useSpecJournalIndexListActions = (dirUrl: string, adapter: SpecJournalIndexListAdapter, afterChanged: () => Promise<void>,): UseActionsResult =>
{
    ...
    const onEdit = useCallback((internalId: string) => { navigate(`${dirUrl}/${internalId}`); }, [navigate, dirUrl]);
    const onCancelBack = useCallback(() => { navigate(dirUrl.replace(/\/Form$/, "/List")); }, [navigate, dirUrl]);
