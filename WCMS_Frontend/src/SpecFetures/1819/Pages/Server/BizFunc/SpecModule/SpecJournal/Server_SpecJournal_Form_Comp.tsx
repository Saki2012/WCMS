...
export const Server_SpecJournal_Form_Comp = (prop: { theme: IBETheme; lang: Lang }) => {
    ...
    const actionsOpt = useMemo<SpecJournalFormActionsOpt>(() => {return { onBackToList };}, [onBackToList]);
    const getData = useSpecJournalFormFetchData({lang: prop.lang, internalId: internalId ?? "", emptyData, actionsOpt, });
    const formProp: FormCompProp = {Title: internalId ? "修改期刊目次" : "新增期刊目次", Theme: prop.theme, IsLoading: getData.isLoading, ErrorList: getData.errors, Actions: getData.rawData.actions,};
    return (
...
const BibliographyComp = (props: { theme: IBETheme; formData: UseFetchFormDataResult<SpecJournalSet> }) => {
    ...
    const addRow = (): void => {
        ...
        const newItem: SpecJournalBibliography = {
            JournalId: parentJournalId,
            RowId: nextRowId,
            Title: "",
            Title_en: "",
    ...
    return (
        <div role="group" className="mt-2">
            {rows.map((r: SpecJournalBibliography, i: number) => {
                ...
                const rowKeys: SpecJournalBibliography = {
                    [SpecJournalBibliographyFields.JournalId]: r?.JournalId,
                    [SpecJournalBibliographyFields.RowId]: r?.RowId,
                };
                            ...
                            <div className="w-100">
                                <div className="col-12 form-group">
                                    <LibTextBox
                                        Style={props.theme.TextBox}
                                        DefaultInputDisplay="請輸入"
                                        {...setField(SpecJournalSetFields.SpecJournalBibliography, SpecJournalBibliographyFields.Title, "string", rowKeys)}
                                    />
                                </div>
                                ...
                                <div className="col-12 form-group">
                                    <LibTextBox
                                        Style={props.theme.TextBox}
                                        DefaultInputDisplay="請輸入"
                                        {...setField(SpecJournalSetFields.SpecJournalBibliography, SpecJournalBibliographyFields.Title_en, "string", rowKeys)}
                                    />
                                </div>
                                ...
                                <div className="col-12 form-group">
                                    <LibTextBox
                                        Style={props.theme.TextBox}
                                        DefaultInputDisplay="請輸入"
                                        {...setField(SpecJournalSetFields.SpecJournalBibliography, SpecJournalBibliographyFields.Url, "string", rowKeys)}
                                    />
                                </div>
                            </div>
                            <button
