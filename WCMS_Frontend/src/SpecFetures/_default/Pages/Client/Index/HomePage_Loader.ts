...
import {AnnouncementDetailFields,AnnouncementFields,BannerDetailFields,BannerDetailInfoFields,
  BannerFields,CategoryDetailFields,CategoryFields,GalleryFields,GalleryInfoFields,
  PGID,TagDataFields,TagDetailFields,} from "@/types/SchemaFields";
...
const buildTagQuery = (progId: string): QueryListParam => {
  return {
    Fields: [
      TagDataFields.InternalId,TagDataFields.TagId,TagDataFields.ProgId,
...
const buildAnnouncementHomeCondition = (p: {lang: Lang;nowIsoLocal: string;categoryIds: string;isTop: boolean;}): string => {
  let cdt = "";
  cdt = LibMerge(" And ", false, cdt, `${AnnouncementFields.Validate_Start} <= ${p.nowIsoLocal}`,
  ...
  // 執行：置頂/非置頂
  cdt = p.isTop ? LibMerge(" And ", false, cdt, `${AnnouncementFields.ContentStatus} & 1`) : LibMerge(" And ", false, cdt, `${AnnouncementFields.ContentStatus} !& 1`);
  ...
  if (p.categoryIds) cdt = LibMerge(" And ", false, cdt, `${AnnouncementFields.Categories} HasAny [${p.categoryIds}]`);
  return cdt;
};
...
const buildGalleryHomeQuery = (p: { condition: string; take: number }): QueryListParam => {
  return {
    Fields: [
      GalleryFields.GalleryId,GalleryFields.InternalId,GalleryFields.Categories,
      GalleryFields.Tags,GalleryFields.ContentStatus,GalleryFields.Validate_Start,
...
export const HomePageLoader = (p: { lang: Lang; }) => async ({ request }: LoaderFunctionArgs): Promise<HomePageLoaderData> => 
{
    ...
    // 執行：一次撈完（SSR）
    const [heroLD, admissionsLD, linksLD, aboutWebLD, aboutPageLD, newsTopLD, newsListLD, newsCateLD, newsTagLD,
        galleryTopLD, galleryListLD, galleryCateLD, galleryTagLD,] = 
        await Promise.all([
    ...
    const newsMerged = takeTopThenFill(newsTopList,newsList,take,x => x.Announcement?.InternalId ?? String(x.Announcement?.AnnouncementId ?? ""),);
    ...
    const galleryMerged = takeTopThenFill(galleryTopList,galleryList,take,x => x.Gallery?.InternalId ?? String(x.Gallery?.GalleryId ?? ""),);
    ...
    // return
    return {
      args: { lang: p.lang, nowIsoLocal, heroBannerInternalId, admissionsBannerInternalId, linksBannerId,
            aboutWebResourceInternalId, aboutPageInternalId, newsCategoryIds, galleryCategoryIds, take,
            linksBannerParam, newsTopParam, newsListParam, newsCateParam, newsTagParam, galleryTopParam,
            galleryListParam, galleryCateParam, galleryTagParam,
        },
      res: {
            rawData: {
                heroBanner, admissionsBanner, linksBanner, aboutWebResource, aboutPage,
                newsTopList, newsList, newsMerged, newsCategories, newsTags,
                galleryTopList, galleryList, galleryMerged, galleryCategories, galleryTags,
            },
        },
    };
};