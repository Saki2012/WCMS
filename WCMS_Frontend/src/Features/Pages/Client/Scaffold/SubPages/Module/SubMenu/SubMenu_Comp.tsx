...
type UseExpandedMenuStateResult = {
...
const CollapseSubMenu_Comp: React.FC<{
    expanded: boolean;
    ...
    const renderParent = (item: MenuItemData, expanded: boolean, depth: number): React.ReactNode => {
        ...
        return (
            <a
                href="#"
                role="button"
                className={clsx("list-group-item", expanded ? "open" : "collapsed")}
                onClick={handleClick}
        ...
        items.map((item, idx) => {
            ...
            return (
                <li key={key} className={clsx("nav-item", hasSubItems(item) && "has-submenu")}>
                    ...
                    {hasSubItems(item) && (
                        <CollapseSubMenu_Comp expanded={expanded}>
                            {renderItems(item.SubItem!, depth + 1)}
                        </CollapseSubMenu_Comp>
                    )}
                </li>
            );
        });
