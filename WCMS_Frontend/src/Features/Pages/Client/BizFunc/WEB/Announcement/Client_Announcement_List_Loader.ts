...
export const useAnnouncementListData = (p: { lang: Lang; opts?: IAnnouncementListOptions; kw?: string; }): UseAnnouncementListDataResult =>
{
    ...
    const template = useMemo(() => createAnnouncementDataQueryTemplate({
        pageState,
        lang: p.lang,
        opts: p.opts,
...
const createAnnouncementDataQueryTemplate = (p: {
...
const resolveGridCellContent = (p: { colKey: string; item: AnnouncementSet; detailTitle: string; finalCount: number; }): string =>
{
    switch (p.colKey)
    {
        case AnnouncementDetailFields.Title: return p.detailTitle;
        case AnnouncementFields.Validate_Start: return formatDate(p.item.Announcement?.Validate_Start) ?? "";
        case AnnouncementFields.Categories: return p.item.Announcement?.Categories ?? "";
        case AnnouncementFields.Tags: return p.item.Announcement?.Tags ?? "";
        case ANNOUNCEMENT_VIEW_COUNT_COL_KEY: return String(p.finalCount);
        default: return "";
    }
};
...
}): ClientDataQueryDataSourceResult<AnnouncementListViewModel> =>
{
    ...
    const gridPropsFromList = buildGridPropsFromList({ lang: p.currentArgs.lang, modelDisplayName: p.grid.modelDisplayName, viewCountModelDisplayName: p.viewCountModel.data, listData: p.grid.list ?? [], viewCountMap: p.viewCountMap, pageNumber: p.grid.pageNumber, totalPages: p.grid.totalPages, onPageChange: p.grid.onPageChange });
    ...
    const rawData: AnnouncementListViewModel = { pageSize: p.currentArgs.pageSize, pageNumber: p.grid.pageNumber, totalPages: p.grid.totalPages, totalCount: p.grid.count, listData: p.grid.list ?? [], categoryData, tagData, viewCountMap: p.viewCountMap, gridPropsFromList, paginatorProps, onPageChange: p.grid.onPageChange };
