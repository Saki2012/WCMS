...
/** 由 list 建立 gridProps */
const buildGridProps = (p: { lang: Lang; listData: WebResourceSet[]; pageNumber: number; totalPages: number; onPageChange: (page: number) => void; }): GridProps =>
{
...
const useWebResourceDataSource = (p: { queryParam: WebResourceQueryParam; loaderData: WebResourceListLoaderData | null; }): ClientDataQueryDataSourceResult<WebResourceListRawData, WebResourceListAdapter> =>
{
    ...
    const gridProps = useMemo(() => buildGridProps({ lang: currentArgs.lang, listData: grid.list ?? [], pageNumber: grid.pageNumber ?? 1, totalPages: grid.totalPages ?? 1, onPageChange: grid.onPageChange }), [
        currentArgs.lang,
        grid.list,
        grid.pageNumber,
        grid.totalPages,
