...
export const Client_WebResourceList_Loader = (p: { lang: Lang; opts: IWebResourceListOptions; }) =>
    async (args: LoaderFunctionArgs): Promise<WebResourceListLoaderData> =>
    {
...
export const useWebResourceListData = (p: { lang: Lang; opts?: IWebResourceListOptions; title?: string; }): UseWebResourceListDataResult =>
{
    ...
    const template = useMemo(() => createWebResourceDataQueryTemplate({
        pageState,
        lang: p.lang,
        opts: p.opts,
        overrides: {
...
const buildCellContent = (p: { item: WebResourceSet; colKey: string; lang: Lang; }) =>
{
    ...
    switch (p.colKey)
    {
        case WebResourceInfoFields.Title: return detail?.Title ?? "";
        case WebResourceInfoFields.ResUrl: return detail?.ResUrl ?? "";
        case WebResourceFields.Categories: return p.item.WebResource?.Categories ?? "";
        default: return "";
    }
};
...
const useWebResourceDataSource = (p: { queryParam: WebResourceQueryParam; loaderData: WebResourceListLoaderData | null; }): ClientDataQueryDataSourceResult<WebResourceListRawData, WebResourceListAdapter> =>
{
    ...
    const gridProps = useMemo(() => buildGridProps({ lang: currentArgs.lang, modelDisplayName: grid.modelDisplayName, listData: grid.list ?? [], pageNumber: grid.pageNumber ?? 1, totalPages: grid.totalPages ?? 1, onPageChange: grid.onPageChange }), [currentArgs.lang, grid.modelDisplayName, grid.list, grid.pageNumber, grid.totalPages, grid.onPageChange]);
    const rawData = useMemo<WebResourceListRawData>(() => ({ count: grid.count ?? 0, listData: grid.list ?? [], categoryMap: category.map ?? {}, gridProps, pageNumber: grid.pageNumber ?? 1, totalPages: grid.totalPages ?? 1, onPageChange: grid.onPageChange, baseParam: grid.param ?? currentArgs.listParam, style: currentArgs.style }), [grid.count, grid.list, category.map, gridProps, grid.pageNumber, grid.totalPages, grid.onPageChange, grid.param, currentArgs.listParam, currentArgs.style]);
    ...
    const refetchData = useCallback(async (): Promise<void> => { await grid.refetchData(); }, [grid.refetchData]);
    const refetchRefData = useCallback(async (): Promise<void> => { await category.refetch(); }, [category.refetch]);
