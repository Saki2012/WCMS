...
const buildInitialData = (
    p: { loaderData: GalleryFormLoaderData | null; internalId: string; fallbackData: GallerySet; },
): ApiLoaderData<string, GallerySet> | null =>
{
    ...
    return { args: p.internalId, apiRes: { IsSuccess: true, Data: p.loaderData.res.dataRes ?? p.fallbackData, SysMessage: [] } };
};

/** 依語系取得標題 */
const getTitleByLang = (p: { data: GallerySet; lang: Lang; }): string =>
{
    const title = p.data.GalleryInfo?.find((item) => item?.Lang?.toLowerCase() === p.lang.toLowerCase())?.Title;
...
export const GalleryForm_Loader = (p: { lang: Lang; }) => async ({ request, params }: LoaderFunctionArgs): Promise<GalleryFormLoaderData> =>
{
    ...
    if (!internalId)
    {
        return { args: { internalId }, res: { dataRes: null, categoryMap: {}, tagMap: {} } };
    }
    const galleryAdapter = GalleryAdapter(ssrApi);
    const categoryAdapter = CategoryAdapter(ssrApi);
    const tagAdapter = TagAdapter(ssrApi);
    const dataLoader = galleryAdapter.loader.createQueryDataLoader({ getInternalId: () => internalId, getApiInstance: () => ssrApi });
    const categoryLoader = categoryAdapter.loader.createMapByProgIdLoader({ progId: PGID.Gallery, lang: p.lang, getApiInstance: () => ssrApi });
    const tagLoader = tagAdapter.loader.createMapByProgIdLoader({ progId: PGID.Gallery, lang: p.lang, getApiInstance: () => ssrApi });
    ...
    return { args: { internalId }, res: { dataRes: dataLD.apiRes.Data ?? null, categoryMap: categoryLD.apiRes.Data ?? {}, tagMap: tagLD.apiRes.Data ?? {} } };
};
...
export const useGalleryFormFetchData = (p: { lang: Lang; emptyData?: GallerySet; }): GalleryFormFetchDataResult =>
{
    ...
    const initialData = useMemo(() => buildInitialData({ loaderData, internalId, fallbackData }), [loaderData, internalId, fallbackData]);

    const queryData = galleryAdapter.hooks.useQueryData({ internalId, initial: initialData, deps: [internalId, p.lang] });
    ...
    const category = categoryAdapter.hooks.useMapByProgId({ progId: PGID.Gallery, lang: p.lang, initial: categoryInitial });
    ...
    const categoryMap = useMemo(() => loaderData?.res.categoryMap ?? category.map ?? {}, [loaderData?.res.categoryMap, category.map]);
    const tagMap = useMemo(() => loaderData?.res.tagMap ?? tag.map ?? {}, [loaderData?.res.tagMap, tag.map]);

    const errorList = useMemo(() =>
    {
        return [queryData.errorText, category.errorText, tag.errorText].filter((item): item is string => Boolean(item));
...
const buildCategoryMapInitial = (p: { loaderData: GalleryFormLoaderData | null; lang: Lang; }): CategoryMapLoaderData | null =>
{
    if (!p.loaderData) return null;

    return { args: { progId: PGID.Gallery, lang: p.lang }, apiRes: { IsSuccess: true, Data: p.loaderData.res.categoryMap ?? {}, SysMessage: [] } };
};

/** 建立 Tag map initial，避免 hydration 首次重抓 */
const buildTagMapInitial = (p: { loaderData: GalleryFormLoaderData | null; lang: Lang; }): TagMapLoaderData | null =>
{
    if (!p.loaderData) return null;

    return { args: { progId: PGID.Gallery, lang: p.lang }, apiRes: { IsSuccess: true, Data: p.loaderData.res.tagMap ?? {}, SysMessage: [] } };
};
