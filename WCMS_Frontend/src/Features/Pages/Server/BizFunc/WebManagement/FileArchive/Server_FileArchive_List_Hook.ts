...
export type FileArchiveListRawData = {
    modelDisplayName: ModelDisplaySchema|null;count: number;list: FileArchiveSet[];
...
export const useFileArchiveListFetchData = (opt: {lang: Lang;kw: string;}): UseFetchDataResult<FileArchiveListRawData,FileArchiveListAdapter> =>
{
    ...
    const adapter = useMemo(() =>{return { FileArchive: FileArchiveAdapter(),Category: CategoryAdapter(),Tag: TagAdapter() };}, []);
    ...
    const grid = adapter.FileArchive.hooks.useQueryGridData({baseParam,deps: [baseParam.Condition ?? "", baseParam.PageSize ?? 0],modelDeps: [opt.lang],onError,});
...
const useFileArchiveListQueryParam = (p: { lang: Lang; kw: string; }): QueryListParam =>
{
    ...
    {
        return [FileArchiveFields.InternalId,FileArchiveFields.FileArchiveId,FileArchiveFields.CategoriesId,FileArchiveFields.ModifyUserId,
                FileArchiveFields.ContentStatus,FileArchiveFields.CreateTime,FileArchiveFields.ModifyTime,
    ...
    const condition = useMemo(() =>
    {
        ...
        if (!!p.kw) cdt = LibMerge(" And ",false,cdt, `${FileArchiveFields._FileArchiveInfo}.${FileArchiveInfoFields.Title} Like ${p.kw}`,);
        return cdt;
    }, [p.lang, p.kw]);
