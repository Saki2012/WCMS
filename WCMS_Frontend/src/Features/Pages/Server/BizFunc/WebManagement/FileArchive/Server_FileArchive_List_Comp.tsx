...
import { useFileArchiveListFetchData, type FileArchiveListRawData } from "./Server_FileArchive_List_Hook";
...
export const Server_FileArchiveListComp = (prop: { title: string; theme: IBETheme; lang: Lang }) => {
    ...
    const gridData = useMemo(() => { return buildFileArchiveGridProps({raw: getData.rawData,lang: prop.lang, crud: {navigate,dirUrl,deleteAsync: cudActions.deleteAsync,afterDelete: getData.refetchData,},});
...
//#region GridProps
type CrudDeps = {navigate: NavigateFunction;dirUrl: string;deleteAsync: (internalId: string) => Promise<ApiResponse<FileArchiveSet>>;afterDelete: () => Promise<void>;};
/** ✅ Announcement 專用：rawData → GridProps（含 ActionCell / Delete confirm） */
const buildFileArchiveGridProps = (opt: {raw: FileArchiveListRawData; lang: Lang;
    ...
    const actions = createGridCrudActions<FileArchiveSet>({onEdit: (internalId) => opt.crud.navigate(`${opt.crud.dirUrl}/${internalId}`),deleteAsync: opt.crud.deleteAsync,afterDelete: opt.crud.afterDelete,});
...
const buildAnnouncementRows = (raw: FileArchiveListRawData, lang: Lang, columns: ColumnConfig[]): GridRow[] => {
    return (raw.list ?? []).map((set) => {
        ...
        const cells: RowCell[] = [
            { col: columns[0], content: mapIdsToText(a?.CategoriesId, raw.categoryMap) },
