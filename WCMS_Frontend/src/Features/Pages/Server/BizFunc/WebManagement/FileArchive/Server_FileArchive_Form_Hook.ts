...
//#region Public
export type FileArchiveFormRawData = {
...
/** ✅ 主入口：Server FileArchive Form 的所有「讀取資料」都集中在這裡 */
export const useFileArchiveFormFetchData = (opt: {lang: Lang;internalId: string;emptyData: FileArchiveSet;actionsOpt: FileArchiveFormActionsOpt;}): UseFetchDataResult<FileArchiveFormRawData, FileArchiveFormAdapter> => {
    const { publish } = useToast();
    const onError = useCallback((e: ApiAdapterError) => {publish({ level: MessageStatus.Error, title: e.messageText });}, [publish]);
    const adapter = useMemo<FileArchiveFormAdapter>(() => {return { FileArchive: FileArchiveAdapter(), Category: CategoryAdapter(), Tag: TagAdapter() };}, []);
    // 執行 function：主資料（ModelDisplayName + QueryData + editable state）
    const formData = useFileArchiveFormDataByAdapter(adapter.FileArchive, opt.internalId, opt.emptyData, onError);
    const actions = useFileArchiveFormActionsByAdapter(adapter.FileArchive,opt.internalId,formData.data,opt.actionsOpt,);
    ...
    const rawData = useMemo<FileArchiveFormRawData>(() => {
...
const useFileArchiveFormDataByAdapter = (adapter: ReturnType<typeof FileArchiveAdapter>,internalId: string,
    empty: FileArchiveSet,onError: (e: ApiAdapterError) => void,): UseFetchFormDataResult<FileArchiveSet> => {
    // 宣告變數
    ...
    const apiRes: ApiResponse<FileArchiveSet> = {IsSuccess: true,Data: empty,SysMessage: [],};
...
const useFileArchiveFormActionsByAdapter = (
  ...
  internalId: string,
  formData: FileArchiveSet,
  opt: FileArchiveFormActionsOpt,
): ServerFormActions => {
  // 宣告變數
  const isNew = useMemo(() => !internalId, [internalId]);
