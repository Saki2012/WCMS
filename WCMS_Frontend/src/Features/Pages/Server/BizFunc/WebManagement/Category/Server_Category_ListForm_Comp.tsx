...
export const Server_CategoryListFormComp = (prop: { progId: PGID; title: string; theme: IBETheme; lang: Lang }) => {
    ...
    const cateEditNode = useMemo(() => (getData.rawData.editForm ? (<CateEditComp theme={prop.theme} formData={getData.rawData.editForm} />) : null),[prop.theme,getData.rawData.editForm],);
    const cateListNode = useMemo(() => (getData.rawData.list ? (<CateListComp theme={prop.theme} cateSets={getData.rawData.list} lang={prop.lang} actions={getData.rawData.actions} />) : null),[prop.theme, getData.rawData.list, prop.lang, getData.rawData.actions],);
    return ( <FormListComp Title={prop.title} SubTitle={prop.title} Theme={prop.theme} isLoading={getData.isLoading} ErrorList={getData.errors} InputControl={cateEditNode} GridItems={cateListNode} Actions={getData.rawData.actions} />);
};
...
const CateEditComp = (props: { theme: IBETheme; formData: UseFetchFormDataResult<CategoryDataSet>; }) => {
    ...
    const tabContent: Record<string, React.ReactNode[]> = rawDetails.reduce<Record<string, React.ReactNode[]>>(
        (compMap, info) => {
            ...
            compMap[langKey] = [
                <LibTextBox Style={props.theme.TextBox} DefaultInputDisplay="請輸入" {...setField(CategoryDataSetFields.CategoryDetail, CategoryDetailFields.CategoryName, "string", rowKeys)} />,
            ];
            return compMap;
        }, {},
    );
