...
//#region Public
export const useCategoryListFormFetchData = (opt: {dirUrl:string;internalId: string; emptyData: CategorySet;lang: Lang;pgId: PGID;}): UseFetchDataResult<CategoryListFormRawData,CategoryListFormAdapter> =>
{
    const { publish } = useToast();
    const onError = useCallback((e: ApiAdapterError) =>{publish({ level: MessageStatus.Error, title: e.messageText });}, [publish]);
    const adapter = useMemo(() =>{return { Category: CategoryAdapter()};}, []);
    ...
    const grid = adapter.Category.hooks.useQueryGridData({baseParam,deps: [baseParam.Condition ?? "", baseParam.PageSize ?? 0],modelDeps: [opt.lang],onError,});
...
//#region Private
const useCategoryListFormDataByAdapter = (adapter: ReturnType<typeof CategoryAdapter>,internalId: string,
    empty: CategorySet,onError: (e: ApiAdapterError) => void,): UseFetchFormDataResult<CategorySet> => {
...
const useCategoryListQueryParam = (p: { lang: Lang; pgId: PGID; }): QueryListParam =>
{
    ...
    {
        return [CategoryFields.InternalId,CategoryFields.CategoryId,
                CategoryFields.ModifyUserId,CategoryFields.ModifyTime,
