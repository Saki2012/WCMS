...
//#region GridProps
type CrudDeps = {navigate: NavigateFunction;dirUrl: string;deleteAsync: (internalId: string) => Promise<ApiResponse<GallerySet>>;afterDelete: () => Promise<void>;};
const buildGalleryGridProps = (opt: {raw: GalleryListRawData; lang: Lang;
    ...
    const actions = createGridCrudActions<GallerySet>({onEdit: (internalId) => opt.crud.navigate(`${opt.crud.dirUrl}/${internalId}`),deleteAsync: opt.crud.deleteAsync,afterDelete: opt.crud.afterDelete,});
...
const buildGalleryRows = (raw: GalleryListRawData, lang: Lang, columns: ColumnConfig[]): GridRow[] => {
    return (raw.list ?? []).map((set) => {
        ...
        const cells: RowCell[] = [
            { col: columns[0], content: coverPicNode },
            { col: columns[1], content: titleNode },
