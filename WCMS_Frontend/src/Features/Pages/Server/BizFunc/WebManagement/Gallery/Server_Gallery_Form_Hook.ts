...
export const useGalleryFormFetchData = (opt: {lang: Lang;internalId: string;emptyData: GallerySet;actionsOpt: GalleryFormActionsOpt;}): UseFetchDataResult<GalleryFormRawData, GalleryFormAdapter> => {
    const { publish } = useToast();
    const onError = useCallback((e: ApiAdapterError) => {publish({ level: MessageStatus.Error, title: e.messageText });}, [publish]);
    const adapter = useMemo<GalleryFormAdapter>(() => {return { Gallery: GalleryAdapter(), Category: CategoryAdapter(), Tag: TagAdapter() };}, []);
...
const useGalleryFormDataByAdapter = (adapter: ReturnType<typeof GalleryAdapter>,internalId: string,
    empty: GallerySet,onError: (e: ApiAdapterError) => void,): UseFetchFormDataResult<GallerySet> => {
    // 宣告變數
