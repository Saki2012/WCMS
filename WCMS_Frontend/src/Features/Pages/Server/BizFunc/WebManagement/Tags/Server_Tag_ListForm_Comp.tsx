...
import { TagDataFields, TagDetailFields, TagSetFields, type PGID } from "@/types/SchemaFields";
...
export const Server_Tag_ListForm_Comp = (prop: { progId: PGID; title: string; theme: IBETheme; lang: Lang }) => {
    ...
    const tagEditNode = useMemo(() => (getData.rawData.editForm ? (<TagEditComp theme={prop.theme} formData={getData.rawData.editForm} />) : null),[prop.theme, getData.rawData.editForm],);
    const tagListNode = useMemo(() => (getData.rawData.list ? (<TagListComp theme={prop.theme} tagSets={getData.rawData.list} lang={prop.lang} actions={getData.rawData.actions} />) : null),[prop.theme, getData.rawData.list, prop.lang, getData.rawData.actions],);
    return <FormListComp Title={prop.title} SubTitle={prop.title} Theme={prop.theme} isLoading={getData.isLoading} ErrorList={getData.errors} InputControl={tagEditNode} GridItems={tagListNode} Actions={getData.rawData.actions}/>
};
...
const TagEditComp = (props: { theme: IBETheme; formData: UseFetchFormDataResult<TagSet>; }) => {
    ...
    const tabContent: Record<string, React.ReactNode[]> = rawDetails.reduce<Record<string, React.ReactNode[]>>(
        (compMap, info) => {
            ...
            compMap[langKey] = [<LibTextBox Style={props.theme.TextBox} DefaultInputDisplay="請輸入" {...setField(TagSetFields.TagDetail, TagDetailFields.TagName, "string", rowKeys)} />,]
            return compMap;
        }, {}
    );
