 ...
 * @returns 
 */
export const PageListComp = ({ title, theme }: { title: string; theme: IBETheme; lang: Lang }) => {
    const dirUrl = useLocation().pathname.replace(/\/List$/, `/Form`);
    const useCategory = useCategoryListData("PageManagement", "zh-tw");
    const usePageList = usePageManagementListData();
    const adjustedGrid = useMemo(() => { return SetAdjustFunction(dirUrl, usePageList.gridProps, usePageList.rawData, useCategory.rawData); }, [dirUrl, usePageList.gridProps, usePageList.rawData, useCategory.rawData]);
