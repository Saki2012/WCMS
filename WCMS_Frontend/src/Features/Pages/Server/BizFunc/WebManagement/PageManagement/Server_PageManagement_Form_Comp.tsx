...
export const Server_PageManagement_Form_Comp = (prop: { theme: IBETheme; lang: Lang }) => {
    ...
    const formProp: FormCompProp = {Title: internalId ? "修改頁面" : "新增頁面",Theme: prop.theme,IsLoading: getData.isLoading, ErrorList: getData.errors, Actions: getData.rawData.actions,};
    return (
        <FormComp prop={formProp}>
            <HeaderComp theme={prop.theme} formData={getData.rawData.formData} catData={getData.rawData.categoryMap} />
...
const HeaderComp = (prop: { theme: IBETheme; formData: UseFetchFormDataResult<PageManagementSet>; catData: Record<string, string> }) => {
    ...
    const components: Record<string, React.ReactNode[]> = {
        Basic: [<LibDropList Style={prop.theme.DropList} Options={prop.catData} {...setField(PageManagementSetFields.PageManagement, PageManagementFields.CategoryId, "string")} />,],
...
const DetailComp = (prop: { theme: IBETheme; formData: UseFetchFormDataResult<PageManagementSet>; lang: Lang }) => {
    ...
    const components: Record<string, React.ReactNode[]> = rawDetails.reduce<Record<string, React.ReactNode[]>>(
        (compMap, info) => {
            ...
            compMap[langKey] = [
                <LibTextBox Style={prop.theme.TextBox} DefaultInputDisplay="請輸入" {...setField(PageManagementSetFields.PageManagementDetail, PageManagementDetailFields.Title, "string", rowKeys)} />,
                <LibTinyMCE Style={prop.theme.TinyMCE} {...setField(PageManagementSetFields.PageManagementDetail, PageManagementDetailFields.Content, "string", rowKeys)} />,
            ];
            return compMap;
