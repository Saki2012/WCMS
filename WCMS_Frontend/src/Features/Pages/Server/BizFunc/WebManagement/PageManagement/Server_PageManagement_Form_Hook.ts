...
export const usePageManagementFormFetchData = (opt: {lang: Lang;internalId: string;emptyData: PageManagementSet;actionsOpt: PageManagementFormActionsOpt;}): UseFetchDataResult<PageManagementFormRawData, PageManagementFormAdapter> => {
    const { publish } = useToast();
    const onError = useCallback((e: ApiAdapterError) => {publish({ level: MessageStatus.Error, title: e.messageText });}, [publish]);
    const adapter = useMemo<PageManagementFormAdapter>(() => {return { PageManagement: PageManagementAdapter(), Category: CategoryAdapter(), Tag: TagAdapter() };}, []);
    ...
    const loadingList = useMemo<boolean[]>(() => {return [Boolean(category.isLoading), Boolean(formData.isLoading)];}, [category.isLoading, formData.isLoading]);
    const errorList = useMemo<(string | null | undefined)[]>(() => {return [category.errorText, formData.error];}, [category.errorText, formData.error]);
    ...
    const rawData = useMemo<PageManagementFormRawData>(() => {return {formData,categoryMap: category.map ?? {},actions};}, [formData, actions, category.map]);
...
const usePageManagementFormDataByAdapter = (adapter: ReturnType<typeof PageManagementAdapter>,internalId: string,
    empty: PageManagementSet,onError: (e: ApiAdapterError) => void,): UseFetchFormDataResult<PageManagementSet> => {
    // 宣告變數
