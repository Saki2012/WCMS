...
export type BannerSliderListRawData = {
    modelDisplayName: ModelDisplaySchema|null;count: number;list: BannerSet[];
...
export const useBannerSliderListFetchData = (opt: {lang: Lang;kw: string;}): UseFetchDataResult<BannerSliderListRawData,BannerSliderListAdapter> =>
{
    ...
    const adapter = useMemo(() =>{return { BannerSlider: BannerSliderAdapter()};}, []);
    ...
    const grid = adapter.BannerSlider.hooks.useQueryGridData({baseParam,deps: [baseParam.Condition ?? "", baseParam.PageSize ?? 0],modelDeps: [opt.lang],onError,});
    // 宣告變數：loading / errors 統一出口
    const isLoading = Boolean(grid.isLoading);
    const errors = useMemo(() =>{const list = [...(grid.errors ?? [])];return list.filter((x): x is string => Boolean(x));}, [grid.errors]);
...
const useBannerSliderListQueryParam = (p: {kw: string; }): QueryListParam =>
{
    ...
    {
        return [BannerFields.InternalId,BannerFields.BannerId,BannerFields.BannerCategoryName,
                BannerFields.ModifyUserId,BannerFields.ModifyTime,
