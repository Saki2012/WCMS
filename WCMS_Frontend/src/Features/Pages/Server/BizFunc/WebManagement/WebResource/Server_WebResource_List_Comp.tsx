...
export const WebResourceListComp = (prop: { title: string; theme: IBETheme; lang: Lang }) => {
    ...
    const gridData = useMemo(() => { return buildWebResourceGridProps({raw: getData.rawData,lang: prop.lang, crud: {navigate,dirUrl,deleteAsync: cudActions.deleteAsync,afterDelete: getData.refetchData,},});
...
//#region GridProps
type CrudDeps = {navigate: NavigateFunction;dirUrl: string;deleteAsync: (internalId: string) => Promise<ApiResponse<WebResourceSet>>;afterDelete: () => Promise<void>;};
/** ✅ Announcement 專用：rawData → GridProps（含 ActionCell / Delete confirm） */
const buildWebResourceGridProps = (opt: {raw: WebResourceListRawData; lang: Lang;
    crud: CrudDeps; can?: (mask: number) => boolean; notifyNoPermission?: (msg: string) => void;confirm?: GridConfirmFn;}): GridProps => {
    const visibleCols = [WebResourceFields.PicId, WebResourceFields.Categories,WebResourceInfoFields.Title,WebResourceFields.ModifyUserId,WebResourceFields.ModifyTime,];
    ...
    const actions = createGridCrudActions<WebResourceSet>({onEdit: (internalId) => opt.crud.navigate(`${opt.crud.dirUrl}/${internalId}`),deleteAsync: opt.crud.deleteAsync,afterDelete: opt.crud.afterDelete,});
