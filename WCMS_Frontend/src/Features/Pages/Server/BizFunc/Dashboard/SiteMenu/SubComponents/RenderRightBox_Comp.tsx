...
export const RenderRightBox = (prop: RenderRightBoxProp) => {
  ...
  const selectedMenuNode = useMemo(() => { return prop.selectedItemEdit?.type === "menu" ? prop.selectedItemEdit.item : null;}, [prop.selectedItemEdit]);
  const selectedMenuItem = selectedMenuNode?.menuItem ?? null;
  ...
  const selectedRowId = selectedMenuItem?.RowId;
  const selectedUrlRow = useMemo(() => { return findSelectedUrlRow(prop.formData.data?.SiteMenu_Item_Url ?? [], selectedSiteIndex, selectedRowId, );}, [prop.formData.data?.SiteMenu_Item_Url, selectedSiteIndex, selectedRowId]);
  const selectedModuleRow = useMemo(() => { return findSelectedModuleRow(prop.formData.data?.SiteMenu_Item_Module ?? [], selectedSiteIndex, selectedRowId,);}, [prop.formData.data?.SiteMenu_Item_Module, selectedSiteIndex, selectedRowId]);
  ...
  useEffect(() => {setTabResetSeed((prev) => prev + 1);}, [selectedRowId, linkType]);
        ...
        <div className="panel">
          <div className="panel-body">
            ...
            <div className="mt-4 overflow-scroll-customize">
              {selectedMenuNode ? (
                <MenuInfoComp theme={prop.theme} selectedItemEdit={selectedMenuNode} formData={prop.formData} setField={setField}
                  siteMenuItems={prop.siteMenuItems} menuUrlType={prop.menuUrlType} windowTarget={prop.windowTarget} modulePageType={prop.modulePageType}
                  bannerDict={prop.bannerDict} moduleDisplayStyle={prop.moduleDisplayStyle} categorySets={prop.categorySets} tagSets={prop.tagSets}
                  pageMap={prop.pageMap} tabResetSeed={tabResetSeed} linkType={linkType} modelKey={modelKey}
                  navType={navType} setLinkType={setLinkType} setModelKey={setModelKey} setNavType={setNavType}
                />
            ...
            <div className="d-flex justify-content-center">
              <button type="button" className="btn btn-custom btn-rounded btn-sm mr-2 mb-2" onClick={prop.action.onSave}>
                儲存
              </button>
              <button type="button" className="btn btn-custom btn-rounded btn-sm mr-2 mb-2" onClick={prop.action.onCancelBack}>
                取消
              </button>
            </div>
          </div>
...
const MenuInfoComp = (prop: MenuInfoCompProps) => {
  ...
  const components = useMemo<Record<string, React.ReactNode[]>>(() => {
    const map: Record<string, React.ReactNode[]> = {
      basic: [
        <BasicSettingTab key="basic" theme={prop.theme} selectedItemEdit={prop.selectedItemEdit}
          formData={prop.formData} setField={prop.setField} itemType={prop.menuUrlType} windowTarget={prop.windowTarget}
    ...
    if (prop.linkType === 2) {
      map.module = [
        <ModuleSettingTab key="module" theme={prop.theme} selectedItemEdit={prop.selectedItemEdit}
          formData={prop.formData} setField={prop.setField} modelKey={prop.modelKey} setModelKey={prop.setModelKey}
          modulePageType={prop.modulePageType} windowTarget={prop.windowTarget} bannerDict={prop.bannerDict}
          moduleDisplayStyle={prop.moduleDisplayStyle} categorySets={prop.categorySets} tagSets={prop.tagSets}
    ...
    if (prop.linkType === 1) {
      map.url = [
        <HyperlinkSettingTab key="url" theme={prop.theme} selectedItemEdit={prop.selectedItemEdit} formData={prop.formData}
          setField={prop.setField} siteMenuItems={prop.siteMenuItems} menuUrlType={prop.menuUrlType} navType={prop.navType}
          setNavType={prop.setNavType} lang={DefaultLang}
        />,
      ];
    }
    return map;
