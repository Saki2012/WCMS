...
import {CategoryDetailFields, CategoryFields, PageManagementDetailFields, PageManagementFields, SiteMenu_IndexFields, SiteMenu_IndexInfoFields, SiteMenu_Item_TitleFields, SiteMenuSetFields, TagDataFields, TagDetailFields,} from "@/types/SchemaFields";
...
//#region Public Types
export interface SiteMenuItem {id: number; name: string; menuItem: SiteMenu_Item, children?: SiteMenuItem[]; }
...
export const useSiteMenuFetchData = (opt: { lang: Lang },): UseFetchDataResult<SiteMenuFetchRawData, SiteMenuFetchAdapter> => {
  ...
  const adapter = useMemo<SiteMenuFetchAdapter>(() => {return {SiteMenu: SiteMenuAdapter(),Category: CategoryAdapter(),Tag: TagAdapter(),Page: PageManagementAdapter(),Banner: BannerSliderAdapter()};}, []);
  // 執行 function：主資料
  ...
  useEnsureLangDetails(main.formData, {headerName: SiteMenuSetFields.SiteMenu_Index,detailName: SiteMenuSetFields.SiteMenu_IndexInfo,parentKeys: [SiteMenu_IndexInfoFields.SiteIndex],preferFirstLang: opt.lang,});
  useEnsureLangDetails(main.formData, {headerName: SiteMenuSetFields.SiteMenu_Item,detailName: SiteMenuSetFields.SiteMenu_Item_Title,parentKeys: [SiteMenu_Item_TitleFields.SiteIndex,SiteMenu_Item_TitleFields.ItemRowId,],preferFirstLang: opt.lang,});
  // 宣告變數：統一出口
  const isLoading = useMemo(() => {return Boolean(main.isLoading || ref.isLoading);}, [main.isLoading, ref.isLoading]);

  const errors = useMemo(() => {return [...main.errors, ...ref.errors].filter((x): x is string => Boolean(x),);}, [main.errors, ref.errors]);
  const siteMenuItems = useMemo(() => {return transSetToItem(main.formData.data, opt.lang); }, [main.formData.data, opt.lang]);
  const rawData = useMemo<SiteMenuFetchRawData>(() => {
  return {internalId: main.internalId,formData: main.formData,actions,
    windowTarget: ref.windowTarget,menuUrlType: ref.menuUrlType,
    modulePageType: ref.modulePageType,moduleDisplayStyle: ref.moduleDisplayStyle,
    bannerDict: ref.bannerDict,categorySets: ref.categorySets,tagSets: ref.tagSets,
    pageMap: ref.pageMap,siteMenuItems,
  };
}, [actions, main.formData, main.internalId, ref, siteMenuItems]);
  const refetchData = useCallback(async () => {await Promise.resolve(main.refetchData());}, [main]);
  const refetchRefData = useCallback(async () => {await Promise.resolve(ref.refetch());}, [ref]);
  // return
...
const useSiteMenuMainDataByAdapter = (adapter: ReturnType<typeof SiteMenuAdapter>,onError: (e: ApiAdapterError) => void,): SiteMenuMainDataResult => {
  ...
  }, [data,model.data,model.errorText,model.isLoading,model.refetch,
    query.errorText,query.isLoading,query.refetch,siteList.errorText,
    siteList.isLoading,siteList.refetch,
  ]);
  const refetchQuery = useCallback(async () => {
    if (!validInternalId) return;
    await query.refetch();
  }, [query, validInternalId]);
  const refetchData = useCallback(async () => {await Promise.all([Promise.resolve(siteList.refetch()),Promise.resolve(refetchQuery),Promise.resolve(model.refetch()),]);}, [model, refetchQuery, siteList]);
  const errors = useMemo(() => {return [siteList.errorText, query.errorText, model.errorText];}, [model.errorText, query.errorText, siteList.errorText]);
  // return
...
const useSiteMenuActionsByAdapter = (adapter: ReturnType<typeof SiteMenuAdapter>,internalId: string | null,formData: UseFetchFormDataResult<SiteMenuSet>,refetchData: () => Promise<void>,): UseActionsResult => {
  const { publish } = useToast();
  const cud = adapter.hooks.useCudActions();
  const onCancelBack = useCallback(() => {void refetchData();}, [refetchData]);
  ...
  return useMemo<UseActionsResult>(() => {
    return {
      isExecuting: cud.isSaving,onSave,onCancelBack,
      onAddNew: () => {},
...
const useSiteMenuRefDataByAdapter = (adapter: SiteMenuFetchAdapter, lang: Lang, onError: (e: ApiAdapterError) => void,): SiteMenuRefDataResult => {
  ...
  // 執行 function：關聯清單
  const category = adapter.Category.hooks.useQueryList({condition: { 
    Fields:[CategoryFields.CategoryId,`${CategoryFields._CategoryDetail}.${CategoryDetailFields.Lang}`,`${CategoryFields._CategoryDetail}.${CategoryDetailFields.CategoryName}`,],
    PageNumber: 0, PageSize: 5000}, deps: [lang], onError,});
  const tag = adapter.Tag.hooks.useQueryList({condition: { Fields:[TagDataFields.TagId,`${TagDataFields._TagDetail}.${TagDetailFields.Lang}`,`${TagDataFields._TagDetail}.${TagDetailFields.TagName}`,],PageNumber: 0, PageSize: 5000}, deps: [lang], onError,});
  const page = adapter.Page.hooks.useQueryList({condition: {Fields:[PageManagementFields.InternalId,`${PageManagementFields._PageManagementDetail}.${PageManagementDetailFields.Lang}`,`${PageManagementFields._PageManagementDetail}.${PageManagementDetailFields.Title}`,],PageNumber: 0, PageSize: 5000 }, deps: [lang], onError,});
  ...
  const isLoading = useMemo(() => { return Boolean(windowTarget.isLoading || menuUrlType.isLoading || modulePageType.isLoading || moduleDisplayStyle.isLoading || category.isLoading || tag.isLoading || page.isLoading || banner.isLoading);
  }, [banner.isLoading, category.isLoading, menuUrlType.isLoading, moduleDisplayStyle.isLoading, modulePageType.isLoading, page.isLoading, tag.isLoading, windowTarget.isLoading,]);
  const errors = useMemo(() => { return [windowTarget.error, menuUrlType.error, modulePageType.error, moduleDisplayStyle.error,category.errorText, tag.errorText, page.errorText, banner.errorText,];
  }, [banner.errorText, category.errorText, menuUrlType.error, moduleDisplayStyle.error, modulePageType.error, page.errorText, tag.errorText, windowTarget.error,]);
  const refetch = useCallback(async () => {await Promise.all([Promise.resolve(category.refetch()), Promise.resolve(tag.refetch()), Promise.resolve(page.refetch()), Promise.resolve(banner.refetch()),]);}, [banner, category, page, tag]);
  return {windowTarget: windowTarget.data, menuUrlType: menuUrlType.data, modulePageType: modulePageType.data, moduleDisplayStyle: moduleDisplayStyle.data,bannerDict, categorySets:category.data, tagSets:tag.data ,pageMap, isLoading, errors, refetch,};
};
...
const useEnumOptions = (enumName: string,): { data: Record<string, string>; isLoading: boolean; error: string | null } => {
  ...
  return useMemo(() => { return { data: src.data ?? {}, isLoading: Boolean(src.isLoading), error: src.error ?? null,};}, [src.data, src.error, src.isLoading]);
};
//#endregion
