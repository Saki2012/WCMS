...
const SiteTitle_Comp = (prop: BasicSettingTabProps) => {
  ...
  const rawDetails = prop.formData.data?.SiteMenu_IndexInfo?.filter((item: SiteMenu_IndexInfo) => {return item.SiteIndex === siteIndex;}) ?? [];
  ...
  const tabContent = dedupDetails.reduce<Record<string, React.ReactNode[]>>((compMap, info) => {
    ...
    compMap[langKey] = [
      <LibTextBox key={`${langKey}_title`} Style={prop.theme.TextBox} DefaultInputDisplay="請輸入" {...prop.setField(SiteMenuSetFields.SiteMenu_IndexInfo, SiteMenu_IndexInfoFields.Title, "string", rowKeys)}/>,
      <LibTextArea key={`${langKey}_desc`} Style={prop.theme.TextBox} DefaultInputDisplay="請輸入" {...prop.setField(SiteMenuSetFields.SiteMenu_IndexInfo, SiteMenu_IndexInfoFields.Description, "string", rowKeys)}/>,
      <LibTinyMCE key={`${langKey}_footer`} Style={prop.theme.CheckBox} {...prop.setField(SiteMenuSetFields.SiteMenu_IndexInfo, SiteMenu_IndexInfoFields.SiteFooter, "string", rowKeys)}/>,
    ];
    return compMap;
  }, {});
...
const SEO_Comp = (prop: BasicSettingTabProps) => {
  return (
    <>
      <LibTextArea key="SEO" Style={prop.theme.TextBox} DefaultInputDisplay="請輸入" {...prop.setField(SiteMenuSetFields.SiteMenu_Index, SiteMenu_IndexFields.GoogleAnalytics, "string")}/>
    </>
  );
};