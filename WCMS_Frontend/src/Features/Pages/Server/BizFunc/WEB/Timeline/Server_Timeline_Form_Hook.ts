...
/** ✅ 主入口：Server Timeline Form 的所有「讀取資料」都集中在這裡 */
export const useTimelineFormFetchData = (
    opt: { lang: Lang; internalId: string; emptyData: TimelineSet; actionsOpt: TimelineFormActionsOpt; },
): UseFetchDataResult<TimelineFormRawData, TimelineFormAdapter> =>
{
    ...
    const adapter = useMemo<TimelineFormAdapter>(() =>
    {
        return { Timeline: TimelineAdapter(), Category: CategoryAdapter(), Tag: TagAdapter() };
    }, []);
    // 執行 function：主資料（ModelDisplayName + QueryData + editable state）
    const formData = useTimelineFormDataByAdapter(adapter.Timeline, opt.internalId, opt.emptyData, onError);
    const actions = useTimelineFormActionsByAdapter(adapter.Timeline, opt.internalId, formData.data, opt.actionsOpt);
    // 執行 function：關聯資料（Category / Tag / ContentStatus）
    const category = adapter.Category.hooks.useMapByProgId({ progId: PGID.Timeline, lang: opt.lang });
    const tag = adapter.Tag.hooks.useMapByProgId({ progId: PGID.Timeline, lang: opt.lang });
...
const useTimelineFormDataByAdapter = (
    adapter: ReturnType<typeof TimelineAdapter>,
    internalId: string,
    empty: TimelineSet,
    onError: (e: ApiAdapterError) => void,
): UseFetchFormDataResult<TimelineSet> =>
{
    // 宣告變數
    const internalKey = internalId || "__new__";
    const isNew = useMemo(() => !internalId, [internalId]);
    const initial = useMemo<ApiLoaderData<string, TimelineSet> | null>(() =>
    {
        // 新建才提供 initial，避免 query "__new__"
        if (!isNew) return null;
        // 建立一個成功回應當作 loader 初始資料
        const apiRes: ApiResponse<TimelineSet> = { IsSuccess: true, Data: empty, SysMessage: [] };
    ...
    const [data, setData] = useState<TimelineSet>(empty);
    useEffect(() =>
    {
...
const useTimelineFormActionsByAdapter = (
    adapter: ReturnType<typeof TimelineAdapter>,
    internalId: string,
    formData: TimelineSet,
    opt: TimelineFormActionsOpt,
): ServerFormActions =>
{
    // 宣告變數
    const isNew = useMemo(() => !internalId, [internalId]);
