...
{
    publishApiMessages(res, publish, fallbackTitle);
...
/** 顯示後端訊息；後端未提供時才使用前端備援文字。 */
const publishApiMessages = <T>(
    res: ApiResponse<T>,
    publish: ReturnType<typeof useToast>["publish"],
    fallbackTitle: string,
): void =>
{
    const messages = res?.SysMessage ?? [];
    if (messages.length > 0)
    {
        emitApiMessages(publish, res, fallbackTitle, "保存失敗");
        return;
    }

    const level = res?.IsSuccess ? MessageStatus.Green : MessageStatus.Error;
    publish({ level, title: res?.IsSuccess ? fallbackTitle : "保存失敗" });
};
