...
import { useAnnouncementListFetchData, type AnnouncementListRawData } from "./Server_Announcement_List_Hook";
...
//#region GridProps
type CrudDeps = {navigate: NavigateFunction;dirUrl: string;deleteAsync: (internalId: string) => Promise<ApiResponse<AnnouncementSet>>;afterDelete: () => Promise<void>;};
...
const buildAnnouncementGridProps = (opt: {raw: AnnouncementListRawData; lang: Lang;
    ...
    const actions = createGridCrudActions<AnnouncementSet>({onEdit: (internalId) => opt.crud.navigate(`${opt.crud.dirUrl}/${internalId}`),deleteAsync: opt.crud.deleteAsync,afterDelete: opt.crud.afterDelete,});
