...
import { BannerDetailFields, BannerFields } from "@/types/SchemaFields";
...
import { type BannerSliderListRawData, useBannerSliderListFetchData } from "./Server_BannerSlider_List_Hook";
...
export const BannerSliderListComp = (prop: { title: string; theme: IBETheme; lang: Lang; }) =>
{
    ...
    {
        return buildBannerSliderGridProps({
            raw: getData.rawData,
            lang: prop.lang,
...
type CrudDeps = {
    ...
    dirUrl: string;
    deleteAsync: (internalId: string) => Promise<ApiResponse<BannerSet>>;
...
const buildBannerSliderRows = (raw: BannerSliderListRawData, columns: ColumnConfig[]): GridRow[] =>
{
    return (raw.list ?? []).map((set) =>
    {
        ...
        const a = set.Banner;
        const picNode = set.BannerDetail?.[0].PicSrcId
            ? (
                <img
                    src={FileManagementAPI.get_Server_Preview_Url(set.BannerDetail?.[0].PicSrcId)}
                    style={{ width: "145px", height: "80px", objectFit: "fill" }}
                />
            )
            : null;
        const cells: RowCell[] = [{ col: columns[0], content: picNode }, { col: columns[1], content: a?.BannerCategoryName }, {
            col: columns[2],
