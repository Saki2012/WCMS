...
import { type PageManagementListRawData, usePageManagementListFetchData } from "./Server_PageManagement_List_Hook";
...
type CrudDeps = {
    ...
    dirUrl: string;
    deleteAsync: (internalId: string) => Promise<ApiResponse<PageManagementSet>>;
...
const buildGalleryRows = (raw: PageManagementListRawData, lang: Lang, columns: ColumnConfig[]): GridRow[] =>
{
    return (raw.list ?? []).map((set) =>
    {
        ...
        const cells: RowCell[] = [{ col: columns[0], content: mapIdsToText(a?.CategoryId, raw.categoryMap) }, { col: columns[1], content: title }, {
            col: columns[2],
