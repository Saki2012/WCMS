...
export const RegisterPage: React.FC = () =>
{
    ...
    const [form, setForm] = useState<IRegisterForm>({ account: "", userName: "", email: "", password: "", confirmPassword: "" });

    const [submitting, setSubmitting] = useState<boolean>(false);
    const [err, setErr] = useState<string | null>(null);

    const pwdMismatch = form.password.length > 0 && form.confirmPassword.length > 0 && form.password !== form.confirmPassword;

    const canSubmit = form.account.trim()
        && form.userName.trim()
        && form.email.trim()
        && form.password.trim()
    ...
    const onChange = (key: keyof IRegisterForm) => (e: React.ChangeEvent<HTMLInputElement>) => setForm((s) => ({ ...s, [key]: e.target.value }));
    ...
    const onSubmit: React.FormEventHandler<HTMLFormElement> = async (e) =>
    {
        ...
        try
        {
            ...
            await UserAPI.create({ UserId: form.account.trim(), UserName: form.userName.trim(), Email: form.email.trim(), Password: form.password });
                        ...
                        <div className="form-content animate__animated animate__fadeInUp delay__125">
                            <div className="page-switcher">
                                ...
                                <ul className="switcher-wrap">
                                    <li>
                                        <LangLink to="/Server/Login" className="switcher-btn">會員登入 Login</LangLink>
                                    </li>
                                </ul>
                            </div>

                            {/* 關閉瀏覽器自動填入；避免帶入使用者名稱/密碼 */}
                            <form className="main-form" onSubmit={onSubmit} noValidate autoComplete="off">
