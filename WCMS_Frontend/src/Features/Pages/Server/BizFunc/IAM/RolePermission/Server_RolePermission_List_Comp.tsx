...
import { type RolePermissionListRawData, useRolePermissionListFetchData } from "./Server_RolePermission_List_Hook";
...
export const Server_RolePermission_Comp = (prop: { title: string; theme: IBETheme; lang: Lang; }) =>
{
    ...
    const searchCompProp: SearchBarProps = { title: "角色搜尋", subTitle: "搜尋角色 ...", settingTitle: "搜尋設定", onSubmit: setKw, onReset: () => setKw("") };
...
/** 欄位定義（順序＝顯示順序） */
const buildColumns = (visibleKeys: ReadonlyArray<readonly [string, string]>, raw: RolePermissionListRawData): ColumnConfig[] =>
{
    // return
    return visibleKeys.map(([tableId, columnId]) =>
    {
        ...
        return { key: columnId, title: hit?.ColumnDisplayName ?? `【${columnId}】` };
    });
};

/** 從 schema 找對應欄位 */
const findSchemaColumn = (schema: ModelDisplaySchema | null, tableId: string, columnId: string) =>
{
    // 宣告變數
