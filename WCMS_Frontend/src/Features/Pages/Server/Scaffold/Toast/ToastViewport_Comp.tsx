...
export const ToastViewport_Comp: React.FC = () => {
    ...
    const startExit = React.useCallback((id: string) => {
        ...
        window.setTimeout(() => {
            ...
            setEntering(prev => { const n = { ...prev }; delete n[id]; return n; });
    ...
    const resumeTimer = React.useCallback((id: string) => {
        const st = timersRef.current.get(id);
        if (!st) return;
        if (st.remaining <= 0) { startExit(id); return; }
        const startAt = Date.now();
    ...
    return (
        ...
        >
            {toasts.map((t, idx) => {
                ...
                let remainingMs = st
                    ? (st.timeoutId
                        ? Math.max(0, st.remaining - (Date.now() - st.startAt))
                        : st.remaining)
                    : 0;
                ...
                return (
                    <section
                        ...
                        onClick={() => { if (t.focusSelector) focusTarget(t.focusSelector, t.id); }}
                        onKeyDown={(e) => {
                            if (!t.focusSelector) return;
                    ...
                    >
                        ...
                        <div>
                            <div style={{ fontWeight: 600 }}>
                                {t.code ? `${t.code} ` : ""}
                                {t.title ??
                                    (t.level === MessageStatus.Error ? "錯誤訊息"
                                        : t.level === MessageStatus.Warning ? "警告訊息"
                                            : t.level === MessageStatus.Green ? "成功訊息"
                                                : "提示訊息")}
                            </div>
