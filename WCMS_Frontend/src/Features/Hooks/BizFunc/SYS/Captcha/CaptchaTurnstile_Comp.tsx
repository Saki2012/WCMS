...
declare global
{
    interface Window { turnstile?: TurnstileClient; }
}
...
export const CaptchaTurnstile_Comp = (props: CaptchaTurnstileProps) =>
{
    ...
    return (
        <div className={props.className} aria-describedby={props.describedBy}>
            ...
            <p id={statusId} className={status === "error" ? "text-danger small mb-0" : "visually-hidden"} role={status === "error" ? "alert" : undefined} aria-live="polite">
...
const clearTurnstileWidget = (widgetId: TurnstileWidgetId | null, container: HTMLDivElement | null): void =>
{
    if (widgetId)
    {
        try { window.turnstile?.remove(widgetId); } catch { /* Turnstile widget may already be removed. */ }
    }
