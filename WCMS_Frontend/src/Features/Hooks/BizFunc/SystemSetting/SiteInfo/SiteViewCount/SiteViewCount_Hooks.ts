...
} from "@/Features/Hooks/BizFunc/SystemSetting/SiteInfo/SiteViewCount/SiteViewCount_Api";
...
export const useSiteViewCount = (options: UseSiteViewCountOptions) =>
{
    ...
    const { runCount } = useCountGuard({
        ...
        action: async (): Promise<ApiResponse<TryCountResultDto | null>> => { return await tryCountSiteViewAsync(siteIndex); },
    });
