...
interface IBuildTagQueryByProgIdParam { progId: string; lang?: Lang; pageSize?: number;}
...
type ExtraLoaders = {
    ...
    createMapByProgIdLoader: (
        opt: ICreateTagMapByProgIdLoader,
    ) => (args: LoaderFunctionArgs) => Promise<TagMapLoaderData>;
};

type ExtraHooks = {
    /** 依 ProgId 取得 Tag map 的 CSR / Hydration hook */
    useMapByProgId: (
        opt: IUseTagMapByProgId,
    ) => TagMapHookResult;
};
...
export const buildTagQueryByProgIdParam = (
    opt: IBuildTagQueryByProgIdParam,
): QueryListParam =>
{
    // 宣告變數
    ...
    const condLang = lang
        ? ` And ${SchemaFields.TagDataFields._TagDetail}.${SchemaFields.TagDetailFields.Lang} = "${lang}"`
        : "";
...
export const formatTagsName = (content: string, tagData: TagSet[], lang: Lang,): string =>
{
    ...
    if (!raw) return "";
    return raw.split(",").map(s => s.trim()).filter(Boolean).map(tagId => tagData?.find(s => String(s.TagData?.TagId) === tagId) ?.TagDetail?.find(d => d.Lang === lang) ?.TagName,).filter((x): x is string => Boolean(x)).join("、");
};

export class TagAdapterImpl extends ApiDataAdapter<TagSet, TagService>
{
    //#region Property
    public declare loader: ApiDataLoaderGroup<TagSet> & ExtraLoaders;
    public declare hooks: ApiDataHookGroup<TagSet> & ExtraHooks;
    //#endregion
    ...
    private useMapByProgId: ExtraHooks["useMapByProgId"] = (opt) =>
    {
        ...
        const args = useMemo<TagMapArgs>(() => { return this.buildTagMapArgs({progId: opt.progId, lang: opt.lang, pageSize: opt.pageSize, }); }, [opt.progId, opt.lang, opt.pageSize]);
        ...
        const map = useMemo<Record<string, string>>(() => { return query.data ?? {}; }, [query.data]);
...
export const TagAdapter = (apiInstance?: AxiosInstance) => { return new TagAdapterImpl((api?: AxiosInstance) => new TagService(api ?? apiInstance),); };