...
export class CategoryAdapterImpl extends ApiDataAdapter<CategorySet, CategoryService>
{
    ...
    private useMapByProgId: ExtraHooks["useMapByProgId"] = (opt) =>
    {
        ...
        {
            return this.buildCategoryMapArgs({ progId: opt.progId, lang: opt.lang, pageSize: opt.pageSize });
    ...
    {
        // 宣告變數
        const query = buildCategoryQueryByProgIdParam({ progId: String(args.progId), lang: args.lang, pageSize: args.pageSize });
        ...
        return { IsSuccess: true, Data: buildCategoryMap(env.Data ?? [], args.lang), SysMessage: env.SysMessage ?? [] };
    }
    // #endregion
}
...
{
    // return
    return new CategoryAdapterImpl((api?: AxiosInstance) => new CategoryService(api ?? apiInstance));
};
