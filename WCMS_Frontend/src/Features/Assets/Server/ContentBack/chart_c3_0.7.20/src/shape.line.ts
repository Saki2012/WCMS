...
import { isDefined, isFunction, isUndefined, isValue } from "./util";
...
{
    return [
        (withTransition ? this.mainArea.transition(transition) : this.mainArea).attr("d", drawArea).style("fill", this.color).style(
            "opacity",
            this.orgAreaOpacity,
        ),
    ];
...
ChartInternal.prototype.updateCircle = function(cx, cy)
{
    ...
    var mainCircleEnter = mainCircle.enter().append("circle").attr("shape-rendering", $$.isStanfordGraphType() ? "crispEdges" : "").attr(
        "class",
        $$.classCircle.bind($$),
    ).attr("cx", cx).attr("cy", cy).attr("r", $$.pointR.bind($$)).style("color", $$.isStanfordGraphType() ? $$.getStanfordPointColor.bind($$) : $$.color);
...
ChartInternal.prototype.redrawCircle = function(cx, cy, withTransition, transition)
{
    ...
    return [
        (withTransition ? $$.mainCircle.transition(transition) : $$.mainCircle).style("opacity", this.opacityForCircle.bind($$)).style(
            "color",
