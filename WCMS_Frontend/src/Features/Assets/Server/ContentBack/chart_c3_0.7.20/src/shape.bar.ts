...
ChartInternal.prototype.redrawBar = function(drawBar, withTransition, transition)
{
    ...
    return [
        (withTransition ? this.mainBar.transition(transition) : this.mainBar).attr("d", drawBar).style("stroke", this.color).style("fill", this.color).style(
            "opacity",
            d => ($$.isTargetToShow(d.id) ? 1 : 0),
        ),
    ];
};
