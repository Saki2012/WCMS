...
{
    return this.addName({ id: d.data.id, value: d.value, ratio: this.getRatio("arc", d), index: d.index });
};
...
ChartInternal.prototype.expandArc = function(targetIds)
{
    ...
    $$.svg.selectAll($$.selectorTargets(targetIds, "." + CLASS.chartArc)).each(function(d)
    {
        ...
        $$.d3.select(this).selectAll("path").transition().duration($$.expandDuration(d.data.id)).attr("d", $$.svgArcExpanded).transition().duration(
            $$.expandDuration(d.data.id) * 2,
        ).attr("d", $$.svgArcExpandedSub).each(function(d)
        {
...
ChartInternal.prototype.updateTargetsForArc = function(targets)
{
    ...
    mainPieEnter.append("text").attr("dy", $$.hasType("gauge") ? "-.1em" : ".35em").style("opacity", 0).style("text-anchor", "middle").style(
        "pointer-events",
        "none",
    );
...
ChartInternal.prototype.redrawArc = function(duration, durationForExit, withTransform)
{
    ...
    if (hasGaugeType)
    {
        ...
        } else
        {
            ...
            }).style("display", config.gauge_labelLine_show ? "" : "none").each(function(d)
            {
                ...
                d3.select(this).attr("x", x).attr("y", y).attr("width", lineLength).attr("height", lineThickness).attr("transform", transform).style(
                    "stroke-dasharray",
    ...
    if (hasGaugeType)
    {
        ...
        backgroundArc.enter().append("path").attr("class", (d, i) => CLASS.chartArcsBackground + " " + CLASS.chartArcsBackground + "-" + i).merge(backgroundArc)
            .attr("d", d1 =>
            {
        ...
        $$.arcs.select("." + CLASS.chartArcsGaugeMin).attr(
            "dx",
            -1 * ($$.innerRadius + ($$.radius - $$.innerRadius) / (config.gauge_fullCircle ? 1 : 2)) + "px",
        ).attr("dy", "1.2em").text(config.gauge_label_show ? $$.textForGaugeMinMax(config.gauge_min, false) : "");
        $$.arcs.select("." + CLASS.chartArcsGaugeMax).attr("dx", $$.innerRadius + ($$.radius - $$.innerRadius) / (config.gauge_fullCircle ? 1 : 2) + "px").attr(
            "dy",
            "1.2em",
