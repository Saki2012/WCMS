...
ChartInternal.prototype.updateStanfordElements = function(duration)
{
    ...
    // Stanford-Lines
    stanfordLine = main.select("." + CLASS.stanfordLines).style("shape-rendering", "geometricprecision").selectAll("." + CLASS.stanfordLine).data(
        config.stanford_lines,
    );
    ...
    stanfordLineEnter.append("line").attr("x1", d => config.axis_rotated ? yvCustom(d, "value_y1") : xvCustom(d, "value_x1")).attr(
        "x2",
        d => config.axis_rotated ? yvCustom(d, "value_y2") : xvCustom(d, "value_x2"),
    ).attr("y1", d => config.axis_rotated ? xvCustom(d, "value_x1") : yvCustom(d, "value_y1")).attr(
        "y2",
        d => config.axis_rotated ? xvCustom(d, "value_x2") : yvCustom(d, "value_y2"),
    ).style("opacity", 0);

    // update
    $$.stanfordLines = stanfordLineEnter.merge(stanfordLine);
    $$.stanfordLines.select("line").transition().duration(duration).attr("x1", d => config.axis_rotated ? yvCustom(d, "value_y1") : xvCustom(d, "value_x1"))
        .attr("x2", d => config.axis_rotated ? yvCustom(d, "value_y2") : xvCustom(d, "value_x2")).attr(
            "y1",
            d => config.axis_rotated ? xvCustom(d, "value_x1") : yvCustom(d, "value_y1"),
        ).attr("y2", d => config.axis_rotated ? xvCustom(d, "value_x2") : yvCustom(d, "value_y2")).style("opacity", 1);
    ...
    stanfordTextEnter.append("text").attr("x", d => (config.axis_rotated ? yvCustom(d, "y") : xvCustom(d, "x"))).attr(
        "y",
    ...
    $$.stanfordTexts.select("text").transition().duration(duration).attr("x", d => (config.axis_rotated ? yvCustom(d, "y") : xvCustom(d, "x"))).attr(
        "y",
        d => (config.axis_rotated ? xvCustom(d, "x") : yvCustom(d, "y")),
    ).text(function(d)
    {
    ...
    stanfordRegionEnter.append("polygon").attr("points", d =>
    {
    ...
    stanfordRegionEnter.append("text").attr("x", d => $$.getCentroid(d.points).x).attr("y", d => $$.getCentroid(d.points).y).style("opacity", 0);

    // update
    $$.stanfordRegions = stanfordRegionEnter.merge(stanfordRegion);
    $$.stanfordRegions.select("polygon").transition().duration(duration).attr("points", d =>
    {
        return d.points.map(value =>
        {
            return [config.axis_rotated ? yvCustom(value, "y") : xvCustom(value, "x"), config.axis_rotated ? xvCustom(value, "x") : yvCustom(value, "y")].join(
    ...
    $$.stanfordRegions.select("text").transition().duration(duration).attr(
        "x",
        d => config.axis_rotated ? yvCustom($$.getCentroid(d.points), "y") : xvCustom($$.getCentroid(d.points), "x"),
    ).attr("y", d => config.axis_rotated ? xvCustom($$.getCentroid(d.points), "x") : yvCustom($$.getCentroid(d.points), "y")).text(function(d)
    {
        if (d.text)
        {
