...
import { isArray, isFunction, isString, isValue, sanitise } from "./util";

ChartInternal.prototype.initTooltip = function()
{
    var $$ = this, config = $$.config, i;
    $$.tooltip = $$.selectChart.style("position", "relative").append("div").attr("class", CLASS.tooltipContainer).style("position", "absolute").style(
        "pointer-events",
        "none",
    ).style("display", "none");
    ...
    if (config.data_groups.length === 0 || config.tooltip_order !== undefined)
    {
        ...
        } else if (isFunction(order))
        {
            ...
            if (config.tooltip_order === undefined)
            {
                ...
                {
                    return order(a ? { id: a.id, values: [a] } : null, b ? { id: b.id, values: [b] } : null);
                };
            }
