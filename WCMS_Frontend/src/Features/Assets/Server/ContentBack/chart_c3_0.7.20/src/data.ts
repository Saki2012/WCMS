...
import { flattenArray, getBBox, hasValue, isArray, isFunction, isNumber, isValue, notEmpty } from "./util";
...
ChartInternal.prototype.getTotalPerIndex = function(axisId)
{
    ...
    $$.data.targets // keep only target that are normalized
        .filter(target => $$.isTargetNormalized(target.id)) // keep only target that are visible
        .filter(target => $$.isTargetToShow(target.id)) // compute sum per axis
        .forEach(target =>
        {
...
ChartInternal.prototype.getTotalDataSum = function()
{
    ...
    const totalDataSum = flattenArray($$.data.targets.filter(target => $$.isTargetToShow(target.id)).map(target => target.values)).map(d => d.value).reduce(
        (p, c) => p + c,
        0,
    );
...
ChartInternal.prototype.findClosestFromTargets = function(targets, pos)
{
    ...
    const candidates = targets.map(t =>
        $$.findClosest(t.values, pos, $$.config.tooltip_horizontal ? $$.horizontalDistance.bind($$) : $$.dist.bind($$), $$.config.point_sensitivity)
    ).filter(v => v);
...
ChartInternal.prototype.findClosest = function(dataPoints, pos, computeDist, minDist = Infinity)
{
    ...
    // find closest bar
    dataPoints.filter(v => v && $$.isBarType(v.id)).forEach(function(v)
    {
        if (!closest)
        {
    ...
    dataPoints.filter(v => v && !$$.isBarType(v.id)).forEach(v =>
    {
...
ChartInternal.prototype.convertValuesToStep = function(values)
{
    ...
    converted[values.length + 1] = { x: converted[values.length].x + 1, value: converted[values.length].value, id: converted[values.length].id };
