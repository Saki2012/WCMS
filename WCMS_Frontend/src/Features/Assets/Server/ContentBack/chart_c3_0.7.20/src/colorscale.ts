...
import { getBBox, isFunction } from "./util";
...
ChartInternal.prototype.drawColorScale = function()
{
    ...
    $$.colorScale.append("g").attr("transform", `translate(0, ${config.stanford_padding.top})`).selectAll("bars").data(points).enter().append("rect").attr(
        "y",
        (d, i) => i * barHeight,
