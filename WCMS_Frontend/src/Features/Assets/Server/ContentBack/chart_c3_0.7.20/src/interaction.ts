...
ChartInternal.prototype.redrawEventRect = function()
{
    ...
    $$.eventRect.attr("x", 0).attr("y", 0).attr("width", $$.width).attr("height", $$.height).on(
        "mouseout",
        ...
        config.interaction_enabled
            ? function()
            {
                ...
                if (closest)
                {
                    const shape = $$.main.selectAll("." + CLASS.shapes + $$.getTargetSelectorSuffix(closest.id)).select(
                        "." + CLASS.shape + "-" + closest.index,
                    );
