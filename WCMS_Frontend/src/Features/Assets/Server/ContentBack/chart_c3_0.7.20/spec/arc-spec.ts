...
describe("c3 chart arc", function()
{
    ...
    describe("unloads correctly", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30], ["data2", 150], ["data3", 120]], type: "pie" }, transition: { duration: 500 } };
        });

        it("unloads without error", function(done)
        {
            chart.load({ columns: [["data2", 30, 20, 50, 40, 60, 50]] });
    ...
    describe("show pie chart", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30], ["data2", 150], ["data3", 120]], type: "pie" } };
        });
        ...
        it("should have correct classes", function()
        {
            var chartArc = d3.select(".c3-chart-arcs"),
                arcs = {
                    data1: chartArc.select(".c3-chart-arc.c3-target.c3-target-data1").select("g.c3-shapes.c3-shapes-data1.c3-arcs.c3-arcs-data1").select(
                        "path.c3-shape.c3-shape.c3-arc.c3-arc-data1",
                    ),
                    data2: chartArc.select(".c3-chart-arc.c3-target.c3-target-data2").select("g.c3-shapes.c3-shapes-data2.c3-arcs.c3-arcs-data2").select(
                        "path.c3-shape.c3-shape.c3-arc.c3-arc-data2",
                    ),
                    data3: chartArc.select(".c3-chart-arc.c3-target.c3-target-data3").select("g.c3-shapes.c3-shapes-data3.c3-arcs.c3-arcs-data3").select(
        ...
        describe("with data id that can be converted to a color", function()
        {
            ...
            describe("with empty pie chart", function()
            {
                beforeAll(function()
                {
                    args = { data: { columns: [["data1", null], ["data2", null], ["data3", null]], type: "pie" } };
                });
                ...
                it("should have correct d attribute", function()
                {
                    var chartArc = d3.select(".c3-chart-arcs"),
                        arcs = {
                            data1: chartArc.select(".c3-chart-arc.c3-target.c3-target-data1").select("g.c3-shapes.c3-shapes-data1.c3-arcs.c3-arcs-data1")
                                .select("path.c3-shape.c3-shape.c3-arc.c3-arc-data1"),
                            data2: chartArc.select(".c3-chart-arc.c3-target.c3-target-data2").select("g.c3-shapes.c3-shapes-data2.c3-arcs.c3-arcs-data2")
                                .select("path.c3-shape.c3-shape.c3-arc.c3-arc-data2"),
                            data3: chartArc.select(".c3-chart-arc.c3-target.c3-target-data3").select("g.c3-shapes.c3-shapes-data3.c3-arcs.c3-arcs-data3")
    ...
    describe("sort pie chart", function()
    {
        ...
        {
            return { data: { order: order, columns: [["data1", 30], ["data2", 150], ["data3", 120]], type: "pie" } };
        };

        var collectArcs = function()
        {
            return d3.selectAll(".c3-arc").data().sort(function(a: any, b: any)
            {
    ...
    describe("config donut chart", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30], ["data2", 150], ["data3", 120]], type: "donut" }, donut: { padAngle: 0.05 } };
        });
    ...
    describe("config pie chart", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30], ["data2", 150], ["data3", 120]], type: "pie" }, pie: { padAngle: 0.05 } };
        });
    ...
    describe("show gauge", function()
    {
        describe("with a 180 degree gauge", function()
        {
            beforeAll(function()
            {
                args = { gauge: { width: 10, max: 10, expand: true }, data: { columns: [["data", 8]], type: "gauge" } };
            });

            it("should have correct d for Pi radian gauge", function()
            {
                var chartArc = d3.select(".c3-chart-arcs"),
                    data = chartArc.select(".c3-chart-arc.c3-target.c3-target-data").select("g.c3-shapes.c3-shapes-data.c3-arcs.c3-arcs-data").select(
                        "path.c3-shape.c3-shape.c3-arc.c3-arc-data",
                    );
        ...
        describe("with a 2 Pi radian gauge that starts at Pi/2", function()
        {
            ...
            {
                args = {
                    gauge: { width: 10, max: 10, expand: true, fullCircle: true, startingAngle: Math.PI / 2 },
                    data: { columns: [["data", 8]], type: "gauge" },
                };
            });
        ...
        describe("with more than one data_column ", function()
        {
            ...
            {
                args = {
                    data: { columns: [["padded1", 100], ["padded2", 90], ["padded3", 50], ["padded4", 20]], type: "gauge" },
                    color: { pattern: ["#FF0000", "#F97600", "#F6C600", "#60B044"], threshold: { values: [30, 80, 95] } },
                };
            });
        ...
        describe("with more than one data value ", function()
        {
            ...
            {
                args = {
                    data: { columns: [["padded1", 40, 60], ["padded2", 100, -10], ["padded3", 0, 50], ["padded4", 20, 0]], type: "gauge" },
                    color: { pattern: ["#FF0000", "#F97600", "#F6C600", "#60B044"], threshold: { values: [30, 80, 95] } },
                };
            });
            var arcColor = ["rgb(96, 176, 68)", "rgb(246, 198, 0)", "rgb(249, 118, 0)", "rgb(255, 0, 0)"];
