import { c3, d3, initChart } from "./c3-helper";
...
describe("c3 chart axis", function()
{
    ...
    var args: any = {
        data: { columns: [["data1", 30, 200, 100, 400, 150, 250], ["data2", 50, 20, 10, 40, 15, 25], ["data3", 150, 120, 110, 140, 115, 125]] },
        axis: { y: { tick: { values: null, count: undefined } }, y2: { tick: { values: null, count: undefined } } },
    };
    ...
    describe("axis x timeseries with seconds", function()
    {
        ...
        {
            args = {
                data: { type: "line", columns: [["epoch", 1401879600000, 1401883200000, 1401886800000], ["y", 1955, 2419, 2262]], xs: { y: "epoch" } },
                axis: { x: { type: "timeseries", min: new Date(1401879600000), max: new Date(1401969600000), localtime: false } },
            };
        });
        ...
        {
            var prevValue;
            d3.select(".c3-axis-x").selectAll("g.tick").each(function(d: any, i)
            {
                if (i !== 0)
                {
        ...
        describe("changing min x time and columns", function()
        {
            beforeAll(function()
            {
                ...
                args.data.columns = [["epoch", 1401876000000, 1401876015000, 1401876030000, 1401876045000, 1401876060000, 1401876075000], [
                    "y",
                    1968,
                    1800,
                    1955,
            ...
            describe("with axis.x.time.format %Y-%m-%d %H:%M:%S", function()
            {
                ...
                {
                    var ticks = d3.select(".c3-axis-x").selectAll("g.tick").selectAll("tspan").each(function(d: any)
                    {
    ...
    describe("axis x timeseries with iso dates", function()
    {
        ...
        {
            args = {
                data: { type: "line", columns: [["epoch", 1527811200000, 1527897600000, 1527984000000], ["y", 1955, 2419, 2262]], xs: { y: "epoch" } },
                axis: {
                    x: {
                        type: "timeseries",
        ...
        var textDates = ["2018-06-01T00:00:00", "2018-06-02T00:00:00", "2018-06-03T00:00:00"];
    ...
    describe("axis y timeseries", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["times", 60000, 120000, 180000, 240000]] }, axis: { y: { type: "timeseries", tick: { time: {} } } } };
        });
        ...
        {
            var prevValue;
            d3.select(".c3-axis-y").selectAll("g.tick").each(function(d: any, i)
            {
                if (i !== 0)
                {
                    var result = d - prevValue;
        ...
        describe("with axis.y.time", function()
        {
            beforeAll(function()
            {
                args.axis.y.tick.time = { type: d3.timeSecond, interval: 60 };
            });
    ...
    describe("axis.y.type", function()
    {
        describe("type=log", function()
        {
            beforeAll(function()
            {
                args = {
                    data: {
                        columns: [["linear", 318, 37, 0, 4, 0, 1], ["log", 318, 37, 0, 4, 0, 1]],
                        type: "bar",
                        axes: { log: "y", linear: "y2" },
                        labels: true,
                    },
                    axis: { y: { type: "log" }, y2: { show: true } },
                };
            });

            it("should have bars from y bigger than y2", function()
            {
                expect((d3.select(".c3-bars-log .c3-bar-5").node() as any).getBBox().height).toBeGreaterThan(
                    (d3.select(".c3-bars-linear .c3-bar-5").node() as any).getBBox().height,
                );
            });
    ...
    describe("axis.x.tick.values", function()
    {
        describe("formatted correctly when negative", function()
        {
            ...
            beforeEach(function()
            {
                args.data = { x: "x", columns: [["x"].concat(xValues as any), ["data1", 30, 200, 100, 400, 150, 250]] };
            });

            it("should not generate whole number for negative values", function()
            {
                var tickValues = [];
                d3.select(".c3-axis-x").selectAll("g.tick").selectAll("tspan").each(function(d: any, i)
                {
        ...
        describe("function is provided", function()
        {
            ...
            beforeEach(function()
            {
                args.axis.x = { tick: { values: tickGenerator } };
    ...
    describe("axis.x.tick.width", function()
    {
        describe("indexed x axis and y/y2 axis", function()
        {
            describe("not rotated", function()
            {
                ...
                {
                    args = {
                        data: { columns: [["data1", 30, 200, 100, 400, 150, 250], ["data2", 50, 20, 10, 40, 15, 25]], axes: { data2: "y2" } },
                        axis: { y2: { show: true } },
                    };
                });
                ...
                {
                    var ticks = chart.internal.main.select(".c3-axis-x").selectAll("g.tick"), expectedX = "0", expectedDy = ".71em";
                    expect(ticks.size()).toBe(6);
                ...
                describe("should set axis.x.tick.format", function()
                {
                    ...
                    {
                        var ticks = chart.internal.main.select(".c3-axis-y").selectAll("g.tick"), expectedX = "-9", expectedDy = "3";
                    ...
                    {
                        var ticks = chart.internal.main.select(".c3-axis-y2").selectAll("g.tick"), expectedX = "9", expectedDy = "3";
                        expect(ticks.size()).toBe(9);
                        ticks.each(function(d)
                        {
                            var tspans = d3.select(this).selectAll("tspan");
                ...
                describe("should set big values in y", function()
                {
                    beforeAll(function()
                    {
                        args.data.columns = [["data1", 3000000000000000, 200, 100, 400, 150, 250], ["data2", 50, 20, 10, 40, 15, 25]];
                    });
        ...
        describe("category axis", function()
        {
            describe("not rotated", function()
            {
                beforeAll(function()
                {
                    args = {
                        data: {
                            x: "x",
                            columns: [["x", "this is a very long tick text on category axis", "cat1", "cat2", "cat3", "cat4", "cat5"], [
                                "data1",
                                30,
                                200,
                                100,
            ...
            describe("option used", function()
            {
                describe("as null", function()
                {
                    ...
                    {
                        var tick = chart.internal.main.select(".c3-axis-x").select("g.tick"), tspans = tick.selectAll("tspan");
                        expect(tspans.size()).toBe(1);
                    });
                });
                ...
                describe("with multilineMax", function()
                {
                    beforeAll(function()
                    {
                        args.axis.x.tick = { multiline: true, multilineMax: 2 };
                    });
                    ...
                    it("should ellipsify x tick properly", function()
                    {
                        ...
                        var expectedTickText = ["this is a very long", "tick text on categ..."];
        ...
        describe("not rotated", function()
        {
            beforeAll(function()
            {
                args = {
                    data: {
                        x: "x",
                        columns: [["x", "category 1", "category 2", "category 3", "category 4", "category 5", "category 6"], [
                            "data1",
                            30,
                            200,
                            100,
            ...
            {
                var box = chart.internal.main.select(".c3-axis-x").node().getBoundingClientRect(), height = chart.internal.getHorizontalAxisHeight("x");
    ...
    describe("axis.y.tick.rotate", function()
    {
        describe("not rotated", function()
        {
            ...
            {
                args = {
                    data: { columns: [["data1", 30, 200, 100, 400, 150, 250, 100, 600], ["data2", 50, 20, 10, 40, 15, 25]] },
                    axis: { rotated: true, y: { tick: { rotate: 45 } } },
                };
            });
            ...
            describe("should set args for x-based data", function()
            {
                beforeAll(function()
                {
                    args = {
                        data: {
                            x: "x",
                            columns: [["x", 10, 20, 100, 110, 200, 1000], ["data1", 30, 200, 100, 400, 150, 250], ["data2", 50, 20, 10, 40, 15, 25], [
                                "data3",
                                150,
                                120,
                                110,
    ...
    describe("axis.y.inner", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30, 200, 100, 400, 150, 250], ["data2", 50, 20, 10, 40, 15, 25]] }, axis: { y: { inner: false } } };
        });
    ...
    describe("axis.x.label", function()
    {
        ...
        {
            args = {
                data: { columns: [["somewhat long 1", 30, 200, 100, 400, 150, 250], ["somewhat long 2", 50, 20, 10, 40, 15, 25]] },
                axis: { x: { show: true, label: { text: "Label of X axis" } } },
            };
        });
