import { compareEpochs, getRegionArea, pointInRegion } from "../src/stanford";
...
describe("c3 stanford tests", function()
{
    ...
    describe("count epochs in region", function()
    {
        beforeAll(function()
        {
            args = { data: { x: "x", y: "y", epochs: "epochs", columns: [["x", 25, 35], ["y", 25, 33], ["epochs", 30, 35]], type: "stanford" } };
        });

        it("should return 0 if the region has no epochs", function()
        {
            var region = [{ x: 0, y: 0 }, { x: 20, y: 0 }, { x: 20, y: 20 }, { x: 0, y: 20 }];
        ...
        {
            var region = [{ x: 0, y: 0 }, { x: 60, y: 0 }, { x: 60, y: 60 }, { x: 0, y: 60 }];
    ...
    {
        var region = [{ x: 0, y: 0 }, { x: 20, y: 0 }, { x: 20, y: 20 }, { x: 20, y: 20 }];
