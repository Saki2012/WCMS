...
describe("drag behavior", function()
{
    ...
    var args = {
        data: {
            x: "x",
            columns: [["x", 10, 30, 45, 50, 70, 100], ["data1", 30, 200, 100, 400, 150, 250], ["data2", 20, 180, 240, 100, 190]],
    ...
    beforeAll(function(done)
    {
        ...
        $$ = chart.internal;
        shapes = $$.main.selectAll("." + $$.CLASS.shapes).selectAll("." + $$.CLASS.shape);
    ...
    {
        var selected = shapes.filter(function()
        {
    ...
    describe("Trigger drag events", function()
    {
        ...
        {
            var unselected = shapes.filter(function()
            {
                return !d3.select(this).classed($$.CLASS.SELECTED);
            }).size();
