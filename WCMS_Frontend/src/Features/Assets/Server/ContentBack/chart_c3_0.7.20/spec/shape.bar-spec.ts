...
describe("c3 chart shape bar", function()
{
    ...
    describe("Path boxes", function()
    {
        beforeAll(function()
        {
            args = { data: { columns: [["data1", 30]], type: "bar" }, bar: { width: { max: 40 } } };
        });

        it("bars should have expected Path Box", function()
        {
            var expected = { x: 279, y: 40, width: 40, height: 387 };

            var shapes = chart.internal.main.selectAll("." + chart.internal.CLASS.shapes).selectAll("." + chart.internal.CLASS.shape);
    ...
    describe("with groups", function()
    {
        describe("with indexed data", function()
        {
            ...
            {
                args = {
                    data: { columns: [["data1", 30, 200, -100, 400, -150, 250], ["data2", 50, 20, 10, 40, 15, 25]], groups: [["data1", "data2"]], type: "bar" },
                };
            });
        ...
        describe("with timeseries data", function()
        {
            beforeAll(function()
            {
                args = {
                    data: {
                        x: "date",
                        columns: [["date", "2012-12-24", "2012-12-25", "2012-12-26", "2012-12-27", "2012-12-28", "2012-12-29"], [
                            "data1",
                            30,
                            200,
                            -100,
    ...
    describe("bar spacing", function()
    {
        var createArgs = function(spacing)
        {
            return {
                size: { width: 500 },
                data: {
                    columns: [["data1", 30, 200, 100], ["data2", 50, 20, 10], ["data3", 150, 120, 110], ["data4", 12, 24, 20]],
                    type: "bar",
                    groups: [["data1", "data4"]],
                },
                bar: { space: spacing },
