...
const LibCheckBox = (prop: ILibCheckBoxProp) => {
  const inputId = useId();
  const uidList = useMemo(() => {return prop.options.map(() => crypto.randomUUID());}, [prop.options]);
          ...
          return (
            <div key={uid} className="col-sm-3 col-6 float-left p-0">
              <div className="custom-control form-check">
                <input className="form-check-input" type="checkbox" id={uid} value={item.itemId} checked={isChecked} onChange={(e) => {
