            ...
            <div className={prop.Style.SelectStyle}>
                <div className="input-group search-box">
                    <input type="text" className={prop.Style.InputStyle} id={inputTextId} placeholder={`${prop.DefaultInputDisplay}${prop.ColumnDisplayName} ...`}
                        value={prop.InputValue ?? ""} onChange={(e) => prop.onNameChange?.(e.target.value)} aria-label={`${prop.ColumnDisplayName} 檔名/說明`} />
                    <button type="button" className="btn btn-custom mb-1" title="刪除附加檔案" onClick={prop.onDelete}><i className="far fa-times"></i></button>
                </div>
