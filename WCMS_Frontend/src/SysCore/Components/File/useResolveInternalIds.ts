...
export interface UseResolveOptions { locale?: string; }
export interface UseResolveResult { html: string; loading: boolean; error?: unknown; }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (alive) setResolvedHtml(out || rawHtml); // ★ 保底
      } catch (e) {
        if (alive) {
          setError(e);
