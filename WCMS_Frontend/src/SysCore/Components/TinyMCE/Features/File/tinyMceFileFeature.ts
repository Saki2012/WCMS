    ...
    ed.windowManager.open({
        ...
        body: {
            ...
            items: [
                ...
                {
                    type: "htmlpanel",
                    html: buildFileLinkModeHtml(radioName),
                },
            ],
        },
        initialData: { files: [], text: "", title: "", target: "" },
        buttons: [{ type: "cancel", text: "取消" }, { type: "submit", text: "插入", primary: true }],
        onChange: (api, details) =>
        {
            if (details.name !== "files") return;
            applyFallbackFileName(api);
        },
        onSubmit: async (api) =>
        {
...
const applyFallbackFileName = (api: { getData: () => unknown; setData: (data: Record<string, unknown>) => void; }) =>
{
    const data = api.getData() as TinyMceFileLinkDialogData;
    const file = data.files?.[0];
    if (!file) return;
...
/** 建立 TinyMCE 目前沒有原生 RadioGroup 的下載/預覽選項。 */
const buildFileLinkModeHtml = (radioName: string): string =>
{
    return `<fieldset class="tox-form__group" style="border:0;padding:0;margin:0">`
        + `<legend style="margin-bottom:8px">連結模式</legend>`
        + `<label style="margin-right:20px"><input type="radio" name="${radioName}" value="download" checked> 下載</label>`
