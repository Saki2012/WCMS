...
type LangLinkBaseProps = {
    ...
    /** 連結開啟方式；未指定時內站預設 _self，外站預設 _blank */
...
/** 判斷是否應改用原生 a，外站與安全特殊協定不走 React Router Link */
const shouldUseNativeAnchor = (to: To): boolean =>
{
    return isExternalHttpUrl(to) || isSafeNativeSchemeUrl(to);
};
...
const resolveTarget = (p: { to: To; target?: React.HTMLAttributeAnchorTarget; }): React.HTMLAttributeAnchorTarget =>
{
    ...
    return isExternalHttpUrl(p.to) ? "_blank" : "_self";
};
